 ...
 * Filename:
 * ---------
 *	 daliovx8000widemipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "daliovx8000widemipiraw_Sensor.h"
...
static int daliovx8000wide_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int daliovx8000wide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt);
static int daliovx8000wide_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int daliovx8000wide_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int daliovx8000wide_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void daliovx8000wide_set_multi_gain(struct subdrv_ctx *ctx, u32 *gains, u16 exp_cnt);
static int daliovx8000wide_set_hdr_tri_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int daliovx8000wide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int daliovx8000wide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void mi_stream(void *arg,bool enable);
static void mi_read_CGRatio(void *arg);
static int daliovx8000wide_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int daliovx8000wide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
/* STRUCT */
static struct mtk_sensor_saturation_info daliovx8000_saturation_info_14bit = {
	.gain_ratio = 3400,
	.OB_pedestal = 64,
...
static struct mtk_sensor_saturation_info daliovx8000_saturation_info_fake14bit = {
	.gain_ratio = 1000,
	.OB_pedestal = 1024,
...
//static u32 daliovx8000_dcg_ratio_table_14bit[] = {16000};

static struct mtk_sensor_saturation_info daliovx8000_saturation_info_10bit = {
	.gain_ratio = 1000,
	.OB_pedestal = 64,
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, daliovx8000wide_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, daliovx8000wide_seamless_switch},
	{SENSOR_FEATURE_SET_AWB_GAIN, daliovx8000wide_set_awb_gain},
	{SENSOR_FEATURE_SET_MULTI_DIG_GAIN, daliovx8000wide_set_multi_dig_gain},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME,daliovx8000wide_set_multi_shutter_frame_length},
	{SENSOR_FEATURE_SET_ESHUTTER, daliovx8000wide_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, daliovx8000wide_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_DUAL_GAIN,daliovx8000wide_set_hdr_tri_gain},
	{SENSOR_FEATURE_SET_HDR_SHUTTER,daliovx8000wide_set_hdr_tri_shutter},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, daliovx8000wide_set_max_framerate_by_scenario},
};
...
#define PRE_MODE  .frame_desc = frame_desc_prev,\
		...
		.mode_setting_table = daliovx8000wide_preview_setting,\
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_preview_setting),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_HD_CAP_MODE,\
		.seamless_switch_mode_setting_table = daliovx8000wide_preview_setting,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_preview_setting),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75190000,\
...
static struct subdrv_mode_struct mode_struct[] = {
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = daliovx8000wide_normal_video_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = daliovx8000wide_hs_video_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_hs_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom1_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom1_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 74832000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom2_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom2_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom3_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom3_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom4_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom4_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom4_setting),
		...
		.saturation_info = &daliovx8000_saturation_info_fake14bit,
		.awb_enabled = true,
	},
	// custom5 mode 9 : ISZ
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom5_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom5_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom5_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom6_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom6_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom6_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom7_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom7_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom7_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom8_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_HD_CAP_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom8_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom8_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75150000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom9_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom9_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// Mode_3 4096x3072_full_crop_quad_10bit_30fps_PD_DT_2048x768_Max_A_gain_15.9375x
	// PD Size = 2048x768
	// Tline = 5.333 us
	// VB = 16.96 ms
	// version:2025-0416_v2.6
	...
	// custom12 mode 16: bokeh slave(same as preview)
	// mode_1
	// PD size = 4096x768
	// Tline = 5.333us
	// VB = 16.949ms
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom13_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom13_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom13_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom13_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75190000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom14_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom14_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom14_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom14_setting),
		...
		.delay_frame = 2,
		.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW14_B,
		.saturation_info = &daliovx8000_saturation_info_14bit,
	},
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom15_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom15_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom15_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom15_setting),
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom17_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom17_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom17_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom17_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// custom18 mode 22 version:2025-0410_v2.5 22: 14bit  video DCG
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom18_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom18_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom18_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom18_setting),
		.hdr_mode = HDR_RAW_DCG_COMPOSE,
		.raw_cnt = 1,
		.exp_cnt = 2,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom19_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom19_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom19_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom19_setting),
		.hdr_mode = PARAM_UNDEFINED,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom20_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom20_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom20_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom20_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
	...
	// version:2025-0416_v2.6
	{
		...
		.mode_setting_table = daliovx8000wide_custom21_setting,
		.mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom21_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = daliovx8000wide_custom21_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(daliovx8000wide_custom21_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75000000,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = DALIOVX8000WIDE_SENSOR_ID,
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = daliovx8000wide_ana_gain_table,
	.ana_gain_table_size = sizeof(daliovx8000wide_ana_gain_table),
	.tuning_iso_base = 50,
	.exposure_def = 0x3D0,
	.exposure_min = 8,
	...
	.exposure_step = 4,
	.exposure_margin = 32,
	.saturation_info = &daliovx8000_saturation_info_10bit,
	...
	.init_setting_table = daliovx8000wide_init_setting,
	.init_setting_len = ARRAY_SIZE(daliovx8000wide_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry daliovx8000wide_mipi_raw_entry = {
    .name = "daliovx8000wide_mipi_raw",
    .id = DALIOVX8000WIDE_SENSOR_ID,
    .pw_seq = pw_seq,
...
static void mi_stream(void *arg,bool enable)
{
	...
	if(enable){
		daliovx8000wide_set_awb_gain(ctx, (u8 *)&g_last_awb_gain, &len);
		i2c_table_write(ctx,daliovx8000wide_stream_on,ARRAY_SIZE(daliovx8000wide_stream_on));
	}else{
		mi_stream_check_stream_off(ctx);
		i2c_table_write(ctx,daliovx8000wide_stream_off,ARRAY_SIZE(daliovx8000wide_stream_off));
	}
}
...
void daliovx8000wide_update_mode_info(struct subdrv_ctx *ctx, enum SENSOR_SCENARIO_ID_ENUM scenario_id)
{
	if (scenario_id >= ctx->s_ctx.sensor_mode_num)
	{
		DRV_LOGE(ctx, "invalid sid:%u, mode_num:%u\n",
...
void daliovx8000wide_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
...
void daliovx8000wide_set_multi_shutter_frame_length_dol(struct subdrv_ctx *ctx,
...
if (set_auto_flicker(ctx, 0) || frame_length || !ctx->s_ctx.reg_addr_auto_extend)
	daliovx8000wide_write_frame_length(ctx, ctx->frame_length);
else if (ctx->s_ctx.reg_addr_auto_extend)
	daliovx8000wide_write_frame_length(ctx, ctx->min_frame_length);
/* write shutter */
switch (exp_cnt) {
case 1:
...
static int daliovx8000wide_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	DRV_LOG(ctx, "daliovx8000wide_set_hdr_tri_shutter");

	daliovx8000wide_set_multi_shutter_frame_length_dol(ctx, values, exp_cnt, 0);
	return 0;
}
...
static int daliovx8000wide_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	u32 frame_length_in_lut[IMGSENSOR_STAGGER_EXPOSURE_CNT] = {0};
	u32 exp_cnt = 0;
	u16 daliovx8000wide_setting_seamless[700] = {0};
	...
	setting_seamless_len = covert_seamless_setting_table(scenario_id, daliovx8000wide_setting_seamless);
	...
	daliovx8000wide_update_mode_info(ctx, scenario_id);

	if (is_dol) {
		i2c_table_write(ctx,daliovx8000wide_seamless_switch_step1_dol,
			ARRAY_SIZE(daliovx8000wide_seamless_switch_step1_dol));
	} else {
		i2c_table_write(ctx,daliovx8000wide_seamless_switch_step1,
			ARRAY_SIZE(daliovx8000wide_seamless_switch_step1));
	}
	...
	if (ctx->s_ctx.mode[scenario_id].seamless_switch_group == 1 ||
		...
		i2c_table_write(ctx,daliovx8000wide_setting_seamless, 2*setting_seamless_len);
	 }

	if (ctx->s_ctx.mode[scenario_id].mi_mode_type == 1)
		daliovx8000wide_set_awb_gain(ctx, (u8 *)&g_last_awb_gain, &retLen);
	...
	if (ae_ctrl) {
		...
		case HDR_RAW_STAGGER:
			daliovx8000wide_set_multi_shutter_frame_length_dol(ctx, (u64 *)&ae_ctrl->exposure, exp_cnt, 0);
		...
		case HDR_RAW_DCG_COMPOSE:
			daliovx8000wide_set_shutter_frame_length(ctx, (u8 *)&ae_ctrl->exposure, len);
			if (ctx->s_ctx.mode[scenario_id].dcg_info.dcg_gain_mode == IMGSENSOR_DCG_DIRECT_MODE) {
				daliovx8000wide_set_multi_gain(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			}
			else
				set_gain(ctx, ae_ctrl->gain.le_gain);
			break;
		default:
			daliovx8000wide_set_shutter(ctx, (u8 *)&ae_ctrl->exposure.le_exposure,len);
			set_gain(ctx, ae_ctrl->gain.le_gain);
			break;
		}
	}
	...
	if (is_dol) {
		i2c_table_write(ctx,daliovx8000wide_seamless_switch_step2_dol,
			ARRAY_SIZE(daliovx8000wide_seamless_switch_step2_dol));
	} else {
		i2c_table_write(ctx,daliovx8000wide_seamless_switch_step2,
			ARRAY_SIZE(daliovx8000wide_seamless_switch_step2));
	}
...
static u16 daliovx8000wide_dgain2reg(struct subdrv_ctx *ctx, u32 dgain)
{
...
static int daliovx8000wide_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	for (i = 0; i < exp_cnt; i++) {
		...
		gains[i] = daliovx8000wide_dgain2reg(ctx, gains[i]);
	}
...
static int daliovx8000wide_set_hdr_tri_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	daliovx8000wide_set_multi_gain(ctx,values, exp_cnt);
	return 0;
}
...
static int daliovx8000wide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	daliovx8000wide_set_shutter_frame_length(ctx, para, len);
	return 0;
}
