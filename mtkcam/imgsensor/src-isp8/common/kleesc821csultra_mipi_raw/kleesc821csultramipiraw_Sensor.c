 ...
 * Filename:
 * ---------
 *     kleesc821csultramipiraw_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
 ...
 *============================================================================
 ****************************************************************************/
#include "kleesc821csultramipiraw_Sensor.h"


#define DEBUG_LOG_EN 0
#define PFX "KLEESC821CSULTRA_camera_sensor"
...
static int kleesc821csultra_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int kleesc821csultra_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int kleesc821csultra_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int kleesc821csultra_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int kleesc821csultra_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);


#define KLEESC821CSULTRA_SENSOR_GAIN_BASE           1024
#define KLEESC821CSULTRA_SENSOR_GAIN_MAX            15.75*1024
#define KLEESC821CSULTRA_SENSOR_GAIN_MAX_VALID_INDEX  6

/* STRUCT */

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, kleesc821csultra_set_test_pattern},
	{SENSOR_FEATURE_SET_GAIN, kleesc821csultra_set_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, kleesc821csultra_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, kleesc821csultra_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME,kleesc821csultra_set_multi_shutter_frame_length},
};
...
// mode 3 : bokeh slave 2816x2112 @24fps
...
// mode 4 : normal video  3264x2040@30fps
...
static struct subdrv_mode_struct mode_struct[] = {
	{	/* 0x12 mode 0 : preview 3264x2448@30fps */
		.mode_setting_table = kleesc821csultra_preview_setting,
		.mode_setting_len = ARRAY_SIZE(kleesc821csultra_preview_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 132000000,
	...
	{	/* 0x0f mode 2 : normal video 3264x1840@30fps */
		.mode_setting_table = kleesc821csultra_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(kleesc821csultra_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 132000000,
	...
	{	/* 0x14 mode 3 : bokeh slave 2816x2112 @24fps */
		.mode_setting_table = kleesc821csultra_bokeh_setting,
		.mode_setting_len = ARRAY_SIZE(kleesc821csultra_bokeh_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 132000000,
	...
	{	/* 0x10 mode 4 : normal video  3264x2040@30fps */
		.mode_setting_table = kleesc821csultra_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(kleesc821csultra_custom1_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 132000000,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_type = 1,
	.ana_gain_step = 1,
	.ana_gain_table = kleesc821csultra_ana_gain_table,
	.ana_gain_table_size = sizeof(kleesc821csultra_ana_gain_table),
	.tuning_iso_base = 50,
	.exposure_def = 0x3D0,
	.exposure_min = 2,
	...
	.init_setting_table = kleesc821csultra_init_setting,
	.init_setting_len = ARRAY_SIZE(kleesc821csultra_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry kleesc821csultra_mipi_raw_entry = {
	.name = "kleesc821csultra_mipi_raw",
	.id = KLEESC821CSULTRA_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void kleesc821csultra_gain2reg(const u64 total_analog_gain, u16* reg_gain_out, u8* analog_reg_gain, u8* digital_fine_reg_gain)
{
	u64 gain = total_analog_gain;
	u8 dig_gain = 0;

	if (gain < KLEESC821CSULTRA_SENSOR_GAIN_BASE) {
		gain = KLEESC821CSULTRA_SENSOR_GAIN_BASE;
	} else if (gain > KLEESC821CSULTRA_SENSOR_GAIN_MAX) {
		gain = KLEESC821CSULTRA_SENSOR_GAIN_MAX;
		dig_gain = 128;
	}

	if ((gain >= 1*KLEESC821CSULTRA_SENSOR_GAIN_BASE) && (gain < 2*KLEESC821CSULTRA_SENSOR_GAIN_BASE)) {
		*reg_gain_out = 0x00;
		*analog_reg_gain = gain * 32 / KLEESC821CSULTRA_SENSOR_GAIN_BASE;
		dig_gain = gain * 4 / (*analog_reg_gain);
	} else if ((gain >= 2*KLEESC821CSULTRA_SENSOR_GAIN_BASE) && (gain < 4*KLEESC821CSULTRA_SENSOR_GAIN_BASE)) {
		*reg_gain_out = 0x01;
		*analog_reg_gain = gain * 32 / (KLEESC821CSULTRA_SENSOR_GAIN_BASE * 2);
		dig_gain = gain * 2 / (*analog_reg_gain);
	} else if ((gain >= 4*KLEESC821CSULTRA_SENSOR_GAIN_BASE) && (gain < 8*KLEESC821CSULTRA_SENSOR_GAIN_BASE)) {
		*reg_gain_out = 0x03;
		*analog_reg_gain = gain * 32 / (KLEESC821CSULTRA_SENSOR_GAIN_BASE * 4);
		dig_gain = gain / (*analog_reg_gain);
	} else if ((gain >= 8*KLEESC821CSULTRA_SENSOR_GAIN_BASE) && (gain < 16*KLEESC821CSULTRA_SENSOR_GAIN_BASE)) {
		*reg_gain_out = 0x07;
		*analog_reg_gain = gain * 32 / (KLEESC821CSULTRA_SENSOR_GAIN_BASE * 8);
		dig_gain = gain / 2 / (*analog_reg_gain);
	} else {
		*reg_gain_out = 0x0f;
		*analog_reg_gain = gain * 32 / (KLEESC821CSULTRA_SENSOR_GAIN_BASE * 16);
		dig_gain = gain / 4 /(*analog_reg_gain);
	}
...
static int kleesc821csultra_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	kleesc821csultra_gain2reg(gain, &reg_gain, &analog_reg_gain, &digital_fine_reg_gain);
...
static int kleesc821csultra_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return kleesc821csultra_set_shutter_frame_length(ctx, para, len);
}
...
static int kleesc821csultra_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	switch (mode) {
	case 5:
		subdrv_i2c_wr_u8(ctx, 0x3721, 0x6d);
		break;
	default:
