...
#ifdef __XIAOMI_CAMERA__
	if (ctx->s_ctx.set_mirror_flip != NULL) {
		ctx->s_ctx.set_mirror_flip((void *) ctx, image_mirror);
		DRV_LOG(ctx, "use custom mirror flip\n");
		return;
	}
#endif
...
#ifdef __XIAOMI_CAMERA__
	if(ctx->s_ctx.mode[ctx->current_scenario_id].hdr_mode == HDR_RAW_DCG_RAW ||  ctx->s_ctx.mode[ctx->current_scenario_id].hdr_mode == HDR_RAW_DCG_COMPOSE){
		if(ctx->s_ctx.mode[ctx->current_scenario_id].dcg_info.dcg_gain_base == IMGSENSOR_DCG_GAIN_LCG_BASE){
			gain = max(gain,
			ctx->s_ctx.mode[ctx->current_scenario_id].multi_exposure_ana_gain_range[IMGSENSOR_EXPOSURE_ME].min);
			gain = min(gain,
			ctx->s_ctx.mode[ctx->current_scenario_id].multi_exposure_ana_gain_range[IMGSENSOR_EXPOSURE_ME].max);
		}else {

			gain = max(gain,
			ctx->s_ctx.mode[ctx->current_scenario_id].multi_exposure_ana_gain_range[IMGSENSOR_EXPOSURE_LE].min);
			gain = min(gain,
			ctx->s_ctx.mode[ctx->current_scenario_id].multi_exposure_ana_gain_range[IMGSENSOR_EXPOSURE_LE].max);
		}
...
extern int lapisimx882_vendor_id;
extern u8 lapisov08f10_vendor_id;
extern u32 lapisov08f10_fusion_len;
extern u32 lapisov08f10_sn_len;
extern char lapisov08f10_fusionID[20];
extern char lapisov08f10_sensorSn[20];
int common_get_vendor_id(struct subdrv_ctx *ctx, u8 *vendor_id)
{
	...
	*vendor_id = vendorId;
	...
	case LAPISS5KHPEWIDE_SENSOR_ID:
		snReg = 30183;
		fusionReg = 16;
		if(SUNNY == vendorId) {
			imgsensorFtm[SENSOR_NUM].fusionID_l = 8;
		...
		break;
	case LAPISIMX882WIDE_SENSOR_ID:
		snReg = 16354;
		fusionReg = 16;
		lapisimx882_vendor_id = vendorId;
		if(OFILM == vendorId) {
		...
		break;
	case LAPISOV20BFRONT_SENSOR_ID:
		if(SUNNY == vendorId) {
	...
	//sn
	if(LAPISOV08F10ULTRA_SENSOR_ID == ctx->s_ctx.sensor_id){
		*vendor_id = lapisov08f10_vendor_id;
		imgsensorFtm[SENSOR_NUM].vendorID = lapisov08f10_vendor_id;
		imgsensorFtm[SENSOR_NUM].fusionID_l = lapisov08f10_fusion_len;
		imgsensorFtm[SENSOR_NUM].snID_l = lapisov08f10_sn_len;

	for(i = 0;i < imgsensorFtm[SENSOR_NUM].snID_l; i++) {
		imgsensorFtm[SENSOR_NUM].sensorSn[i] = lapisov08f10_sensorSn[i];
	}
	//fusionID
	for(i = 0;i < imgsensorFtm[SENSOR_NUM].fusionID_l; i++) {
		imgsensorFtm[SENSOR_NUM].fusionID[i] = lapisov08f10_fusionID[i];
	}
	}else{
...
void sensor_init(struct subdrv_ctx *ctx)
{
	...
	/* write init setting */
	if (ctx->s_ctx.init_setting_table != NULL || ctx->s_ctx.s_mi_init_setting != NULL) {
