...
static int seninf_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable && !ctx->streaming) {
		if (unlikely(*(ctx->core->seninf_vsync_debug_flag))) {
			...
			ctx->bypass_record_cammux_info_flag = 1;
			g_seninf_ops->_set_all_cam_mux_vsync_irq(ctx, 1);
			dev_info(ctx->dev,
				"[%s][%d]vsync irq enabled,bypass_record_cammux_info_flag:%u\n",
				__func__, __LINE__, ctx->bypass_record_cammux_info_flag);
	...
	if (enable && core->err_detect_init_flag) {
		spin_lock_irqsave(&core->spinlock_irq, flags);
		dev_info(ctx->dev,
			"[%s] _enable_stream_err_detect enable(%d)\n",
			__func__, core->err_detect_init_flag);
		g_seninf_ops->_enable_stream_err_detect(ctx);
		ctx->bypass_record_cammux_info_flag = 0;
		dev_info(ctx->dev,
			"[%s][%d]bypass_record_cammux_info_flag:%u\n",
			__func__, __LINE__, ctx->bypass_record_cammux_info_flag);
		spin_unlock_irqrestore(&core->spinlock_irq, flags);
	}
...
static int seninf_close(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (!ctx->open_refcnt) {
		...
		if (core->pwr_refcnt_for_aov &&
			...
			!(core->current_sensor_id < 0)) {
			dev_info(ctx->dev, "%s error: aov sensor streaming on scp now\n", __func__);
		} else {
			/* clear aov_ctx */
...
static int runtime_suspend(struct device *dev)
{
	...
	if (core->refcnt == -1 &&
		...
		!(core->current_sensor_id < 0)) {
		power_handle = true;
		seninf_logi(ctx, "error: aov sensor streaming on scp now, set power_handle = %d\n", power_handle);
	}
	...
	if (core->refcnt >= 0) {
		...
		seninf_logi(ctx, "error: power count error! cnt(%d)\n", core->refcnt);
...
static int runtime_resume(struct device *dev)
{
	...
	core->refcnt++;
	if (core->refcnt > 0 || power_handle) {
		if (core->refcnt == 1 || power_handle) {
			dev_info(dev,
				"[%s] 1st user(%d),cnt(%d)\n",
		...
		if (core->refcnt == 1 || power_handle) {
			if (core->pwr_refcnt_for_aov &&
				...
				(core->current_sensor_id != core->aov_sensor_id)) {
				seninf_logi(ctx,
				"aov sensor streaming on scp now, disable mux/cammux instead of aov path\n");
	...
	enum AOV_DEINIT_TYPE aov_seninf_deinit_type)
{
	...
	if (!power_handle) {
		...
		power_handle = false;
		dev_info(ctx->dev, "[%s] set power_handle = %d, do not power on\n", __func__, power_handle);
	}
