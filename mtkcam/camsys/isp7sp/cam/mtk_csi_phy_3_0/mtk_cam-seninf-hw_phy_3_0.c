...
static int mtk_cam_seninf_disable_cammux(struct seninf_ctx *ctx, int cam_mux)
{
	...
	if (core->err_detect_init_flag)
		seninf_logd(ctx,
		"disable vsync irq bypass when enable cammux vsync irq for debugging. (err detect use)\n");
	else
...
static int mtk_cam_seninf_disable_all_mux(struct seninf_ctx *ctx)
{
	...
	for (i = 0; i < (_seninf_ops->mux_num - 1); i++) {
			...
			struct mtk_cam_seninf_vsync_info *vsync_info)
{
	...
	for (i = 0; i < vsync_info->used_seninf_num; i++) {
		SENINF_SNPRINTF(buf, len,
			" seninf%d:(csi_irq:0x%x)",
			vsync_info->used_seninf[i],
			vsync_info->seninf_irq_st[i]);
	}
	...
	for (j = 0; j < vsync_info->used_mux_num; j++) {
		SENINF_SNPRINTF(buf, len,
			" mux:0x%x:(irq:0x%x)",
			vsync_info->used_mux[j],
			vsync_info->seninf_mux_irq_st[j]);
		if (vsync_info->seninf_mux_irq_st[j] & (0x1))
			SENINF_SNPRINTF(buf, len,
				" seninf mux:0x%x overrun:(irq:0x%x)",
				vsync_info->used_mux[j],
				vsync_info->seninf_mux_irq_st[j]);
	}
	...
	struct mtk_cam_seninf_vsync_info *vsync_info)
{
	...
	list_for_each_entry(ctx_, &core->list, list) {
		if (ctx_->bypass_record_cammux_info_flag) {
			dev_info(ctx_->dev,
				"[%s]bypass_record_cammux_info_flag:%u ",
				__func__, ctx_->bypass_record_cammux_info_flag);
			continue;
		}
		if (ctx_->streaming && ctx_->power_status_flag) {
	...
	struct mtk_cam_seninf_vsync_info *vsync_info)
{
	...
	list_for_each_entry(ctx_, &core->list, list) {
		...
		if (ctx_->streaming && ctx_->power_status_flag) {
			...
			base_seninf = ctx_->reg_if_csi2[(uint32_t)ctx_->seninfIdx];
			vsync_info->seninf_irq_st[vsync_info->used_seninf_num] =
				SENINF_READ_REG(base_seninf, SENINF_CSI2_IRQ_STATUS);
			if (vsync_info->seninf_irq_st[vsync_info->used_seninf_num] & (0x10000000))
				SENINF_WRITE_REG(base_seninf, SENINF_CSI2_IRQ_STATUS, 0xffffffff);
			vsync_info->used_seninf[vsync_info->used_seninf_num++] = ctx_->seninfIdx;
			...
			for (i = 0; i < ctx_->vcinfo.cnt; i++) {
				...
				for (j = 0; j < vc->dest_cnt; j++) {
					...
					vsync_info->used_mux[used_mux] = dest->mux;
					vsync_info->seninf_mux_irq_st[used_mux] =
						SENINF_READ_REG(pmux, SENINF_MUX_IRQ_STATUS);
					if (vsync_info->seninf_mux_irq_st[used_mux] & (0x1))
						SENINF_WRITE_REG(pmux,
							SENINF_MUX_IRQ_STATUS, 0xffffffff);
					if (core->csi_irq_en_flag &&
					(vsync_info->seninf_mux_irq_st[used_mux] & (0x3))) {
			...
			vsync_info->used_mux_num = used_mux;
		}
	}
}
