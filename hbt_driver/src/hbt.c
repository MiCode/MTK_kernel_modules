...
static long hbt_compat_ioctl(struct hbt_compat_ioctl __user *argp)
{
	 ...
	 */
	if (args.cmd == 0xc0306201 || // Magic number of 64B BINDER_WRITE_READ
	    args.cmd == 0xc0383e04 || args.cmd == 0xc0303e03 || // dma-buf/sync_file
	    args.cmd == 0x4020800f || args.cmd == 0xc008803c || // gpu_mali
	    args.cmd == 0x4010802f || args.cmd == 0x40088025 || // gpu_mali
	    args.cmd == 0x802c || args.cmd == 0x40048019 || // gpu_mali
	    args.cmd == 0xc0186766 || args.cmd == 0xc0186765 || // gtype
	    args.cmd == 0xc0186767 || args.cmd == 0x40286703 || // gtype
	    args.cmd == 0x40286701 || args.cmd == 0xc0186704 || // gtype
	    args.cmd == 0xc0186769 || args.cmd == 0x4028670e) { // gtype
	} else {
		set_32bit(true);
	}
...
#endif
{
	...
	err = buf->error = verify_dirent_name(name, namlen);
	if (unlikely(buf->error))
		goto error_out;
	buf->error = -EINVAL; /* only used if we fail.. */
	if (reclen > buf->count && (err = -EINVAL))
		goto error_out;
...
static struct miscdevice hbt_device2 = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "vhbt",
	.fops = &hbt_fops,
	.mode = 0666,
...
static int __init hbt_init(void)
{
	int ret;
	if ((ret = misc_register(&hbt_device)) != 0)
		return ret;
	if ((ret = misc_register(&hbt_device2)) != 0)
		return ret;

	return 0;
}

static void __exit hbt_exit(void)
{
