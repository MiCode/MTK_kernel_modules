...
// MIUI ADD
#if CFG_QOS_ENABLE
static uint8_t getTos(struct sk_buff *skb) {
	uint8_t tos = 0;
...
/* increase tx priority if tos is not be set and skb-> priority is be set */
static void update_tx_priority(struct sk_buff *skb,
	...
	priority =  cfg80211_classify8021d(skb);
#else
	priority = cfg80211_classify8021d(skb, qos_map);
#endif

	/* In the following three situations, use the original priority:
	 * 1. neither IPv4 nor IPv6 package
	 * 2. is IPv4 or IPv6 package and the value of TOS is not 0
	 * 3. skb->priority is not a valid value
	 * Tip:  DSCP is superior to skb->priority
	*/
	if ((skb->protocol != htons(ETH_P_IP) && skb->protocol != htons(ETH_P_IPV6)) ||
		getTos(skb) || skb->priority >= WMM_MAX_UP_VALUE) {
...
// MIUI ADD
#if CFG_QOS_ENABLE
	update_tx_priority(skb, qos_map);
#else
// END
#if KERNEL_VERSION(3, 14, 0) > CFG80211_VERSION_CODE
