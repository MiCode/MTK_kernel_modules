...
static u_int8_t isChannelInStaDFsChannels(struct ADAPTER *prAdapter,
	uint8_t ucChannelNum,
	struct CHANNEL_BAND_INFO *prStaConnectedChnlInfo,
	uint32_t ucStaDfsNums)
{
	uint32_t i;

	/* channel is not dfs for P2P GO, if STA already connected to it*/
	for (i = 0; i < ucStaDfsNums; i++) {
		if (prStaConnectedChnlInfo[i].ucChannelNum == ucChannelNum)
			return TRUE;
	}
	return FALSE;
}
	...
	uint32_t ucStaDfsNums)
{
	...
	if (iface & (IFACE_MODE_STA | IFACE_MODE_P2P_CLIENT | IFACE_MODE_TDLS |
		...
		if (eBand == BAND_5G) {
			if (fgDFS) {
				if (!isChannelInStaDFsChannels(prAdapter,
					ucChannelNum,
	...
	const void *data, int data_len)
{
	...
	struct CHANNEL_BAND_INFO rStaConnectedChnlInfo[BSSID_NUM];
	uint32_t ucStaDfsNums = 0;
	...
	if (rStatus != WLAN_STATUS_SUCCESS)
		DBGLOG(REQ, ERROR, "failed to get sta connected chnls\n");
