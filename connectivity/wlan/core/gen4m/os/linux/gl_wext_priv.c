...
#if CFG_SUPPORT_ANT_HALF
#include "gl_kal.h"
#endif

#if CFG_SUPPORT_CONNSYS_THERMAL
...
#if CFG_SUPPORT_CONNSYS_THERMAL
        case PRIV_CMD_SET_THERMAL_LEVEL:
#endif
#if CFG_SUPPORT_ANT_HALF
	case PRIV_CMD_SET_ANT_HALF_CTRL:
                {
                        char *pCommand = NULL;
...
int priv_driver_sap_fixed_rate_change(struct net_device *prNetDev,
				  char *pcCommand, int i4TotalLen)
{
	int32_t i4Argc = 0;
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = {0};
	...
	struct GLUE_INFO *prGlueInfo = NULL;
	uint8_t ucRoleIdx = 0, ucBssIdx = 0;
	struct MSG_P2P_SAP_FIXED_RATE_CHANGE *prP2pSapFixedRateChangeMsg =
		(struct MSG_P2P_SAP_FIXED_RATE_CHANGE *) NULL;
	struct WIFI_VAR *prWifiVar;
	uint32_t u4Ret = 0;
	uint32_t fgIsEnable = 0;
	...
	if (i4Argc >= 2) {
		u4Ret = kalkStrtou32(apcArgv[1], 0, &fgIsEnable);
	}
	DBGLOG(REQ, INFO, "u4Ret is %d\n", u4Ret);

	prP2pSapFixedRateChangeMsg = (struct MSG_P2P_SAP_FIXED_RATE_CHANGE*)
		cnmMemAlloc(prGlueInfo->prAdapter,
			RAM_TYPE_MSG, sizeof(struct MSG_P2P_SAP_FIXED_RATE_CHANGE));
	if (prP2pSapFixedRateChangeMsg == NULL)
		return i4BytesWritten;

	prP2pSapFixedRateChangeMsg->rMsgHdr.eMsgId =
		MID_MNY_P2P_SAP_FIXED_RATE_CHANGE;
	prP2pSapFixedRateChangeMsg->ucRoleIdx = ucRoleIdx;
	prP2pSapFixedRateChangeMsg->fgFixedRateChange =
		fgIsEnable;

	mboxSendMsg(prGlueInfo->prAdapter,
		MBOX_ID_0,
		(struct MSG_HDR *) prP2pSapFixedRateChangeMsg,
		MSG_SEND_METHOD_BUF);
...
#if CFG_SUPPORT_ANT_HALF
/* set ant half */
int priv_driver_set_ant_half(struct net_device *prNetDev,
                                  char *pcCommand, int i4TotalLen)
{
       uint32_t ant_half = 0;
       char *chain_siso = "SET_CHIP SmartGear 9 1";
       char *chain_mimo = "SET_CHIP SmartGear 9 2";
       ...
       DBGLOG(REQ, INFO, "command is %s\n", pcCommand);
       wlanCfgParseArgument(pcCommand, &i4Argc, pcArgv);
       DBGLOG(REQ, LOUD, "argc is %i\n", i4Argc);
       if (i4Argc <= 1) {
           DBGLOG(REQ, ERROR, "Argc(%d) ERR: ant_half\n", i4Argc);
           return -1;
       }
       u4Ret = kalkStrtou32(pcArgv[1], 0, &ant_half);
       if (u4Ret)
       {
           DBGLOG(REQ, ERROR, "parse ant_half error u4Ret=%d\n", u4Ret);
           return -1;
       }
       DBGLOG(REQ, INFO, "%s ant_half = %d \n", __func__, ant_half);

       i4TotalLen = kalStrLen(chain_siso) + 1;
       if (ant_half == 0) {
           priv_driver_set_chip_config(prNetDev, chain_mimo, i4TotalLen);
       } else if (ant_half == 1) {
           priv_driver_set_chip_config(prNetDev, chain_siso, i4TotalLen);
       } else {
           DBGLOG(REQ, ERROR, "%s ant_half err, value = %d \n", __func__, ant_half);
           return -1;
       }
...
#if CFG_SUPPORT_CONNSYS_THERMAL
int priv_driver_set_thermal_level(struct net_device *prNetDev,
                                  char *pcCommand, int i4TotalLen)
{
       uint32_t thermal_level = 0;
       int conv_level = 0;
       int thermal_result = 0;
       enum conn_pwr_drv_type type;
       ...
       if (i4Argc <= 1) {
           DBGLOG(REQ, ERROR, "Argc(%d) ERR: set thermal level\n", i4Argc);
           return -1;
       }

       u4Ret = kalkStrtou32(apcArgv[1], 0, &thermal_level);
       if (u4Ret)
           DBGLOG(REQ, ERROR, "parse set thermal level error u4Ret=%d\n", u4Ret);

       DBGLOG(REQ, INFO, "%s thermal_level = %d \n", __func__, thermal_level);

       if(thermal_level >= 10) {
           type = CONN_PRW_DRV_ALL;
           conv_level = thermal_level - 10;
       } else if ( thermal_level >= 0 && thermal_level <= 9) {
           type = CONN_PWR_DRV_WIFI;
           conv_level = thermal_level;
       } else {
           return -1;
       }
       ...
       switch (conv_level) {
       case 0:
           level = CONN_PWR_THR_LV_0;
           break;
       case 1:
           level = CONN_PWR_THR_LV_1;
           break;
       case 2:
           level = CONN_PWR_THR_LV_2;
           break;
       case 3:
           level = CONN_PWR_THR_LV_2;
           break;
       ...
       thermal_result = conn_pwr_set_customer_level(type, level);
       if(thermal_result < 0) {
           DBGLOG(REQ, ERROR, "%s return thermal_result, value = %d \n", __func__, thermal_result);
           return -1;
       }
       return 0;
}
