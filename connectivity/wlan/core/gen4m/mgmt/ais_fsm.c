	...
	struct BSS_DESC_SET *prBssDescSet, uint8_t ucBssIndex)
{
	...
	if (roam->eReason == ROAMING_REASON_BTM &&
		...
		if (prAisSpecificBssInfo != NULL) {
			prEssLink = &prAisSpecificBssInfo->rCurEssLink;
		}

		if (prAdapter->fgEnLowLatencyMode && roam->eReason == ROAMING_REASON_TX_ERR
			&& ((roam->ucRcpi > 70) || (prEssLink && prEssLink->u4NumElem == 1))) {
			DBGLOG(AIS, INFO,
				"[Roaming] Game mode with special conditions, skip full scan\n");
		} else {
			DBGLOG(AIS, INFO,
...
void aisFsmAddBlockList(struct ADAPTER *prAdapter,
	uint8_t ucBssIndex, uint16_t u2DeauthReason)
{
	struct AIS_FSM_INFO *ais = aisGetAisFsmInfo(prAdapter, ucBssIndex);
	struct WIFI_VAR *prWifiVar = &prAdapter->rWifiVar;
	uint8_t i;
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		if (prWifiVar->fgDeauthBlkByLink &&
		    prAisBssInfo && prAisBssInfo->ucBssIndex != ucBssIndex) {
			DBGLOG(AIS, TRACE,
				"Only add deauth link[%d] to blocklist!\n",
				i);
			continue;
		}
			 ...
			 struct MSG_HDR *prMsgHdr)
{
	...
	if (ucReasonOfDisconnect == DISCONNECT_REASON_CODE_DEAUTHENTICATED ||
		...
		aisFsmAddBlockList(prAdapter, ucBssIndex, u2DeauthReason);
