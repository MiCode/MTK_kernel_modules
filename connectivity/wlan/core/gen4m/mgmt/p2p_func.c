		...
		enum ENUM_P2P_FILTER_SCENARIO_TYPE eFilterScnario)
{
	...
	ucNumAliveNonSapBss = cnmGetAliveNonSapBssInfo(
						prAdapter, aliveNonSapBss);
#if (CFG_SUPPORT_CONNAC1X || CFG_SUPPORT_CONNAC2X || CFG_SUPPORT_CONNAC3X)
	/*same rf band scc*/
...
void p2pFuncSapFixedRateChange(
		struct ADAPTER *prAdapter,
		uint16_t u2BtProfile)
{
	...
	if (prP2pBssInfo != NULL &&
		...
		if (prP2pBssInfo2 == NULL ||
			prP2pBssInfo2->eBand != BAND_2G4)
			return;
		else
			prP2pBssInfo = prP2pBssInfo2;
	} else if (prP2pBssInfo == NULL)
		return;
		...
		enum ENUM_P2P_FILTER_SCENARIO_TYPE *eFilterScnario)
{
	...
	if (ucChnlNum != 0 &&
		...
		/* subband boundary definition is */
		/* decribed with freq/ ch edge */
		...
		for (j = 0; j < MAX_BSSID_NUM; j++) {
			...
			if (IS_BSS_ALIVE(prAdapter, prBssInfo) &&
				...
				prSapSwitchCand[k].eHwBand =
					prBssInfo->eHwBandIdx;
				DBGLOG(P2P, INFO,
					"[cand gen]extend alive bss: %u channel\n",
					j);
