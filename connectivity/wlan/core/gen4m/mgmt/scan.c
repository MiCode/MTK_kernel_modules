		...
		struct MSG_SCN_SCAN_REQ_V2 *prScanReqMsg)
{
	...
	// MIUI ADD: Start
	uint32_t u4FullToPartialPeriod = CFG_SCAN_FULL2PARTIAL_PERIOD;
	// MIUI END
	...
	if (fgIsOnlineScan && (u4ScanChannelNum == 0 ||
		...
		// MIUI ADD: Start
		if (prAdapter != NULL) {
			u4FullToPartialPeriod = prAdapter->rWifiVar.u4FullToPartialPeriod;
			log_dbg(SCN, INFO, "update FullToPartialPeriod to %d\n", u4FullToPartialPeriod);
		}
		// MIUI END
		/* Do full scan when
		...
		*/
		if (prScanInfo->u4LastFullScanTime == 0 ||
			...
			// MIUI ADD: MODIFY
			SEC_TO_SYSTIME(u4FullToPartialPeriod))) ||
			fgIsHighAccuracy) {
