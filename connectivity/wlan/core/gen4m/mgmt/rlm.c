...
uint32_t rlmCalculateMiOuiIELen(
        struct ADAPTER *prAdapter,
        uint8_t ucBssIndex,
        struct STA_RECORD *prStaRec)
{
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief This function is used to generate MI Vendor Specific OUI
 *
 * \param[in]
 *
 * \return none
 */
/*----------------------------------------------------------------------------*/
void rlmGenerateMiOuiIE(struct ADAPTER *prAdapter,
         struct MSDU_INFO *prMsduInfo)
{
        ...
        uint8_t *pucBuffer;
        uint16_t len;
        uint8_t aucMiSapOui[] = VENDOR_OUI_MI;

        if ((prAdapter == NULL) || (prMsduInfo == NULL))
                return;

        if (prAdapter->rWifiVar.ucMiSapOui == FEATURE_DISABLED)
                return;
        ...
        if (prAdapter->rWifiVar.ucMiSapOui == FEATURE_ENABLED) {
                DBGLOG(INIT, WARN, "Sap OUI is enable\n");
                MI_OUI_IE(pucBuffer)->ucId = ELEM_ID_VENDOR;
                MI_OUI_IE(pucBuffer)->ucLength = ELEM_MIN_LEN_MI_OUI;
                MI_OUI_IE(pucBuffer)->aucOui[0] = aucMiSapOui[0];
                MI_OUI_IE(pucBuffer)->aucOui[1] = aucMiSapOui[1];
                MI_OUI_IE(pucBuffer)->aucOui[2] = aucMiSapOui[2];
                MI_OUI_IE(pucBuffer)->aucCapability[0] = 0x06;
                MI_OUI_IE(pucBuffer)->aucCapability[1] = 0x01;
                MI_OUI_IE(pucBuffer)->aucCapability[2] = 0x01;
                MI_OUI_IE(pucBuffer)->aucCapability[3] = 0x03;
                MI_OUI_IE(pucBuffer)->aucCapability[4] = 0x01;
                MI_OUI_IE(pucBuffer)->aucCapability[5] = 0x00;
                MI_OUI_IE(pucBuffer)->aucCapability[6] = 0x00;
