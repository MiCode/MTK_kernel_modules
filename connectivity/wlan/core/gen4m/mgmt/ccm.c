				...
				const char *pucSrcFunc)
{
	...
	if (prWifiVar->eP2pCcmMode != P2P_CCM_MODE_DISABLE) {
		...
		DBGLOG(CCM, WARN, "P2P CCM is disabled, skip to check GO\n");
	}
			 ...
			 struct RF_CHANNEL_INFO *prChnlInfo2)
{
	...
	/* For tc8sp only */
	if (prChnlInfo_l->u4CenterFreq1 > P2P_5G_L_UPPER_BOUND ||
	    prChnlInfo_h->u4CenterFreq1 < P2P_5G_H_LOWER_BOUND) {
		DBGLOG(CCM, TRACE,
		       "not support A+A if not 5GL+5GH, cf_l=%u, cf_h=%u\n",
		       prChnlInfo_l->u4CenterFreq1,
		       prChnlInfo_h->u4CenterFreq1);
		return FALSE;
	}
