		...
		struct QUE *prFragmentedQue)
{
	...
	MLR_DBGLOG(prAdapter, TX, INFO, "MLR frag - Start\n");
	/* KAL_REC_TIME_START(); */
	 ...
	 */
	MLR_DBGLOG(prAdapter, TX, INFO, "MLR frag - End\n");
		  ...
		  struct WIFI_EVENT *prEvent)
{
	...
	if ((IS_BSS_AIS(prBssInfo) &&
		...
		if ((prStaRec->ucMlrState == MLR_STATE_IDLE
		...
		DBGLOG(NIC, INFO,
			   "MLR event - mode[%d->%d] state[%d->%d] frag[%d->%d] reason[%d->%d] sendU[%d]\n",
			   prStaRec->ucMlrMode,
			   prEvtMlrFsmUpdate->ucMlrMode,
			   prStaRec->ucMlrState,
			   prEvtMlrFsmUpdate->ucMlrState,
		...
		DBGLOG(NIC, INFO,
			   "MLR event - BssIdx[%d]WlanIdx[%d]StaRecIdx[%d] M:S:F:SB[0x%02x, %d, %d, 0x%02x(R=%d)], Bitmap[0x%02x](0x%02x & 0x%02x), EnTxFrag=%d\n",
			   prBssInfo->ucBssIndex,
	...
	MLR_DBGLOG(prAdapter, NIC, INFO,
		"MLR event - BssIdx[%d]WlanIdx[%d]StaRecIdx[%d] Connected[%d] M:S:F:SB[0x%02x, %d, %d, 0x%02x]\n",
		prBssInfo->ucBssIndex,
		prEvtMlrFsmUpdate->u2WlanIdx,
		ucStaIdx,
