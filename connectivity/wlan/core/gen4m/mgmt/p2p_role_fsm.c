	...
	uint8_t ucLinkIdx)
{
	...
	kalMemZero(prP2pSpecificBssInfo, sizeof(*prP2pSpecificBssInfo));
...
void p2pRoleFsmRunEventFixedRateChange(struct ADAPTER *prAdapter,
		struct MSG_HDR *prMsgHdr)
{
	struct P2P_ROLE_FSM_INFO *prP2pRoleFsmInfo =
		(struct P2P_ROLE_FSM_INFO *) NULL;
	struct MSG_P2P_SAP_FIXED_RATE_CHANGE *prP2pSapFixedRateMsg =
		(struct MSG_P2P_SAP_FIXED_RATE_CHANGE *) NULL;
	struct BSS_INFO *prP2pBssInfo = (struct BSS_INFO *) NULL;

	prP2pSapFixedRateMsg = (struct MSG_P2P_SAP_FIXED_RATE_CHANGE *) prMsgHdr;

	prP2pRoleFsmInfo =
		P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter,
			prP2pSapFixedRateMsg->ucRoleIdx);

	DBGLOG(P2P, TRACE,
		"with Role(%d) and En(%u)\n",
		prP2pSapFixedRateMsg->ucRoleIdx,
		prP2pSapFixedRateMsg->fgFixedRateChange);
	...
	if ((!prP2pSapFixedRateMsg->fgFixedRateChange) &&
		(prP2pBssInfo->u2BSSBasicRateSet ==
		BASIC_RATE_SET_HR_DSSS_ERP_COEX ||
		prP2pBssInfo->u2BSSBasicRateSet ==
		BASIC_RATE_SET_ERP_COEX)) {
		nicUpdateBssEx(prAdapter,
			prP2pBssInfo->ucBssIndex,
			FALSE);
		if (prP2pBssInfo->u2BSSBasicRateSet ==
			BASIC_RATE_SET_HR_DSSS_ERP_COEX)
			prP2pBssInfo->u2BSSBasicRateSet =
				BASIC_RATE_SET_HR_DSSS_ERP;
		else if (prP2pBssInfo->u2BSSBasicRateSet ==
			BASIC_RATE_SET_ERP_COEX)
			prP2pBssInfo->u2BSSBasicRateSet =
				BASIC_RATE_SET_ERP;
	} else if ((prP2pBssInfo->u2BSSBasicRateSet ==
