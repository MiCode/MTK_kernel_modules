	...
	struct BSS_DESC *prBssDesc, enum ROAM_TYPE eRoamType)
{
	...
	if (prBssDesc->eBand == BAND_2G4)
		u2Score = 0;
	else if ((prBssDesc->eBand == BAND_5G && prAdapter->fgEnable5GBand)
#if (CFG_SUPPORT_WIFI_6G == 1)
	...
	if (cRssi <= prAdapter->rWifiVar.cVeryLowRSSI)
		u2Score = 0;
	else if (cRssi < LOW_RSSI_FOR_5G_BAND) {
		diff = LOW_RSSI_FOR_5G_BAND - prAdapter->rWifiVar.cVeryLowRSSI;
		u2Score = (u2Score / diff) *
			(cRssi - prAdapter->rWifiVar.cVeryLowRSSI);
	}
	...
	int8_t cCandidateRssi, int8_t cCurrentRssi)
{
	...
	if (eCurrentBand == BAND_5G
		...
		if (cCurrentRssi > LOW_RSSI_FOR_5G_BAND &&
		    cCurrentRssi - cCandidateRssi >= RSSI_DIFF_SML_STEP)
			return TRUE;
	}
	return FALSE;
}
	...
	int8_t cCandidateRssi, int8_t cCurrentRssi)
{
	...
	if (eCurrentBand == BAND_2G4) {
		if (cCandidateRssi < LOW_RSSI_FOR_5G_BAND &&
		    cCurrentRssi - cCandidateRssi >= RSSI_DIFF_BIG_STEP)
			return TRUE;
	}
#if (CFG_SUPPORT_WIFI_6G == 1)
	/* Target AP is 6G, replace candidate AP if target AP is good */
		...
		if (cCandidateRssi < LOW_RSSI_FOR_5G_BAND &&
		    cCurrentRssi - cCandidateRssi >= RSSI_DIFF_MED_STEP)
			return TRUE;
	}
#endif
	return FALSE;
	...
	int8_t cCandidateRssi, int8_t cCurrentRssi)
{
	...
	if (eCurrentBand == BAND_2G4) {
		if (cCandidateRssi < LOW_RSSI_FOR_5G_BAND &&
		    cCurrentRssi - cCandidateRssi >= RSSI_DIFF_BIG_STEP)
			return TRUE;
	/* Target AP is 5G, replace candidate AP if target AP has better rssi */
	} else if (eCurrentBand == BAND_5G) {
		if (cCandidateRssi < LOW_RSSI_FOR_5G_BAND &&
		    cCurrentRssi - cCandidateRssi >= RSSI_DIFF_MED_STEP)
			return TRUE;
	}
...
#if (CFG_SUPPORT_LOW_RSSI_THR_FOR_5G == 1)
		...
		*/
		if (prCandBss->eBand != prCurrBss->eBand &&
			...
			if ((prCurrBss->eBand == BAND_5G &&
				cCurrRssi >= prWifiVar->iLowRssiThr5gBand) ||
				(prCandBss->eBand == BAND_5G &&
				cCandRssi >= prWifiVar->iLowRssiThr5gBand))
#else
			if ((prCurrBss->eBand == BAND_5G &&
	...
	if (prBssDesc->prBlock) {
		...
		if (prBssDesc->prBlock->fgDeauthLastTime &&
		    ((prApsInfo->u2EssApNum == 1
				...
				&& !(prAdapter->rWifiVar.ucReconnectDeauthAP &&
				(prBssDesc->prBlock->u2DeauthReason == REASON_CODE_CLASS_2_ERR ||
				prBssDesc->prBlock->u2DeauthReason == REASON_CODE_CLASS_3_ERR ||
				prBssDesc->prBlock->u2DeauthReason == REASON_CODE_UNSPECIFIED))
#endif
	...
	uint8_t ucBssIndex)
{
	...
	u2ScoreScanMiss = CALCULATE_SCORE_BY_MISS_CNT(prAdapter,
		prBssDesc, eRoamType);
	u2ScoreBand = scanCalculateScoreByBand(prAdapter, prBssDesc, eRoamType);
