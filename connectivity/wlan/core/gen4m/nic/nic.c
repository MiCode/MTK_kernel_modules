...
// MIUI ADD
#if CFG_QOS_ENABLE
u_int8_t checkCwAndAifsnParam(uint16_t u2CwMin, uint16_t u2CwMax,
				uint16_t u2Aifsn)
{
	if (OUT_OF_MAX_VALUE(u2CwMin, CW_MAX_VALUE)
			|| OUT_OF_MAX_VALUE(u2CwMax, CW_MAX_VALUE)
			|| INVALID_WMM_PARAM(u2Aifsn, AIFSN_MIN_VALUE, AIFSN_MAX_VALUE)) {
		return FALSE;
	} else {
		return TRUE;
	}
...
u_int8_t checkTxopParam(uint16_t u2TxopLimit)
{
	if (OUT_OF_MAX_VALUE(u2TxopLimit, TXOPLIMIT_MAX_VALUE)) {
		return FALSE;
	} else {
		return TRUE;
	}
...
void __setWmmParams(struct CMD_UPDATE_WMM_PARMS *prCmdUpdateWmmParms,
				enum ENUM_AC ac, uint16_t u2CwMin, uint16_t u2CwMax,
				uint16_t u2Aifsn)
{
	if (!prCmdUpdateWmmParms) {
		DBGLOG(QM, ERROR, "prCmdUpdateWmmParms is null.\n");
		return;
	}
	prCmdUpdateWmmParms->arACQueParms[ac].u2CWmin = u2CwMin;
	prCmdUpdateWmmParms->arACQueParms[ac].u2CWmax = u2CwMax;
	prCmdUpdateWmmParms->arACQueParms[ac].u2Aifsn = u2Aifsn;
}

void __updateWmmParams(struct CMD_UPDATE_WMM_PARMS *prCmdUpdateWmmParms,
				enum ENUM_AC ac, uint16_t u2CwMin, uint16_t u2CwMax,
				uint16_t u2Aifsn, uint16_t u2TxopLimit)
{
	if (checkCwAndAifsnParam(u2CwMin, u2CwMax, u2Aifsn)) {
			__setWmmParams(prCmdUpdateWmmParms, ac, u2CwMin, u2CwMax, u2Aifsn);
			DBGLOG(QM, INFO, "update params: %d(%d:%d:%d)", ac,
				u2CwMin, u2CwMax, u2Aifsn);
	} else {
			DBGLOG(QM, ERROR, "invalid params (%d)", ac);
	}

	if (checkTxopParam(u2TxopLimit)) {
		DBGLOG(QM, INFO, "update param: %d(%d)", ac, u2TxopLimit);
		prCmdUpdateWmmParms->arACQueParms[ac].u2TxopLimit = u2TxopLimit;
	} else {
		DBGLOG(QM, ERROR, "invalid txop param (%d)", ac);
	}
}

void updateWmmParamsByXm(struct ADAPTER *prAdapter,
				struct CMD_UPDATE_WMM_PARMS *prCmdUpdateWmmParms)
{
	uint16_t u2VoCwMin, u2VoCwMax, u2VoTxopLimit, u2VoAifsn;
	uint16_t u2ViCwMin, u2ViCwMax, u2ViTxopLimit, u2ViAifsn;
	uint16_t u2BkCwMin, u2BkCwMax, u2BkTxopLimit, u2BkAifsn;

	if (!prAdapter || !prCmdUpdateWmmParms) {
		DBGLOG(QM, ERROR, "some pointer is null.\n");
		return;
	}

	DBGLOG(QM, TRACE, "Update WMM params\n");

	if (prAdapter->rWifiVar.fgEnableWmmParams == 1) {
		u2VoCwMin = prAdapter->rWifiVar.u2WmmParamVoCWmin;
		u2VoCwMax = prAdapter->rWifiVar.u2WmmParamVoCWmax;
		u2VoTxopLimit = prAdapter->rWifiVar.u2WmmParamVoTxopLimit;
		u2VoAifsn = prAdapter->rWifiVar.u2WmmParamVoAifsn;
		u2ViCwMin = prAdapter->rWifiVar.u2WmmParamViCWmin;
		u2ViCwMax = prAdapter->rWifiVar.u2WmmParamViCWmax;
		u2ViTxopLimit = prAdapter->rWifiVar.u2WmmParamViTxopLimit;
		u2ViAifsn = prAdapter->rWifiVar.u2WmmParamViAifsn;
		u2BkCwMin = prAdapter->rWifiVar.u2WmmParamBkCWmin;
		u2BkCwMax = prAdapter->rWifiVar.u2WmmParamBkCWmax;
		u2BkTxopLimit = prAdapter->rWifiVar.u2WmmParamBkTxopLimit;
		u2BkAifsn = prAdapter->rWifiVar.u2WmmParamBkAifsn;

		__updateWmmParams(prCmdUpdateWmmParms, AC3, u2VoCwMin, u2VoCwMax,
			u2VoAifsn, u2VoTxopLimit);
		__updateWmmParams(prCmdUpdateWmmParms, AC2, u2ViCwMin, u2ViCwMax,
			u2ViAifsn, u2ViTxopLimit);
		__updateWmmParams(prCmdUpdateWmmParms, AC1, u2BkCwMin, u2BkCwMax,
			u2BkAifsn, u2BkTxopLimit);
	} else {
		DBGLOG(QM, TRACE, "wmm config is disabled.\n");
	}
}
#endif
// END
...
// MIUI ADD
#if CFG_QOS_ENABLE
	/* Use wmm params set by wifi.cfg if "EnableWmmParams" cfg is enabled.
	 * CwMin、CwMax、Aifsn are considered a set of params, if any of them is an
	 * invalid value, the entire set of params will not be set.
	 * TxopLimit can be set individually if it's a valid value.
	 */
	updateWmmParamsByXm(prAdapter, &rCmdUpdateWmmParms);
#endif
// END
