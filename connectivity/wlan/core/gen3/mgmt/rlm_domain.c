...
static void PwrLmtTblArbitrator(INT_8 *target,
	INT_8 *compare,
	UINT_32 size)
{
	UINT_8 i = 0;
			...
			if (prCmd->ucNum != 0) {
				for (k = 0; k < prCmd->ucNum; k++) {
					if (prCmdPwrLimit->ucCentralCh ==
						 ...
						 */
						PwrLmtTblArbitrator(&prCmdPwrLimit->cPwrLimitCCK,
							   &g_rRlmPowerLimitConfiguration[i].aucPwrLimit[0],
							   PWR_LIMIT_NUM);
				...
				if (k == prCmd->ucNum) {
					...
					PwrLmtTblArbitrator(&prCmdPwrLimit->cPwrLimitCCK,
						   &g_rRlmPowerLimitConfiguration[i].aucPwrLimit[0], PWR_LIMIT_NUM);
					prCmd->ucNum++;
