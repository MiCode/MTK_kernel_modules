...
WLAN_STATUS kalFirmwareOpen(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	UINT_8 ucCurEcoVer = wlanGetEcoVersion(
				      prGlueInfo->prAdapter);
#if defined(MT6631)
	UINT_8 aucFwName[128];
	UINT_16 u2ChipID = nicGetChipID(prGlueInfo->prAdapter);
#endif
	UINT_8 fgResult = FALSE;
	UINT_32 ret;

	ASSERT(prGlueInfo->prDev);
	...
	for (ucNameIdx = 0; apucNameTable[ucNameIdx]; ucNameIdx++) {
		if (kalSnprintf(aucFwName, sizeof(aucFwName), "%s",
				apucNameTable[ucNameIdx]) < 0)
			continue;
			...
			if (kalSnprintf(aucFwName + strlen(aucFwName), sizeof(aucFwName) -
				strlen(aucFwName), "%x", u2ChipID) < 0)
				continue;
#endif
		/*
		 ...
		 */
		ret = request_firmware(&fw_entry, aucFwName, prGlueInfo->prDev);
		if (ret) {
			DBGLOG(INIT, TRACE,
...
kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf, IN UINT_32 u4BufLen)
{
	...
	case WLAN_STATUS_BSS_CH_SWITCH:
		{
			struct cfg80211_chan_def chan = {0};
			...
			cfg80211_ch_switch_notify(prGlueInfo->prDevHandler, &chan, 0, 0);
#else
		 ...
		 IN PUINT_8 pucIPv4Addr, IN UINT_32 u4NumIPv4Addr, IN PUINT_8 pucIPv6Addr, IN UINT_32 u4NumIPv6Addr)
{
	...
	u4Len += sizeof(PARAM_NETWORK_ADDRESS_LIST);
	/* IPv4 */
	u4Len += (OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress) + IPV4_ADDR_LEN) * u4NumIPv4Addr;
	/* IPv6 */
	u4Len += (OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress) + IPV6_ADDR_LEN) * u4NumIPv6Addr;
	...
	prParamNetAddr = (P_PARAM_NETWORK_ADDRESS)(prParamNetAddrList + 1);
	for (i = 0; i < u4NumIPv4Addr; i++) {
		prParamNetAddr->u2AddressType = PARAM_PROTOCOL_ID_TCP_IP;
		prParamNetAddr->u2AddressLength = IPV4_ADDR_LEN;
		kalMemCopy(prParamNetAddr->aucAddress, &pucIPv4Addr[i * IPV4_ADDR_LEN], IPV4_ADDR_LEN);
	...
	for (i = 0; i < u4NumIPv6Addr; i++) {
		...
		kalMemCopy(prParamNetAddr->aucAddress, &pucIPv6Addr[i * IPV6_ADDR_LEN], IPV6_ADDR_LEN);
...
int kalMetInitProcfs(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	proc_create(PROC_MET_PROF_CTRL, 0, pMetProcDir, (const struct proc_ops *)&rMetProcCtrlFops);
	proc_create(PROC_MET_PROF_PORT, 0, pMetProcDir, (const struct proc_ops *)&rMetProcPortFops);
