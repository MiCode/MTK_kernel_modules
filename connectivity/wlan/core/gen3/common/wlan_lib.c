...
wlanAdapterStart(IN P_ADAPTER_T prAdapter, IN P_REG_INFO_T prRegInfo)
{
	...
	do {
		...
		prAdapter->rAcpiState = WLAN_ACPI_STATE_D0;
...
WLAN_STATUS wlanAdapterStop(IN P_ADAPTER_T prAdapter)
{
	...
	if (prAdapter->rAcpiState == WLAN_ACPI_STATE_D0 &&
			...
			if (EQUAL_MAC_ADDR(prAdapter->rWlanInfo.rCurrBssId.arMacAddress,
				...
				if (i != 0) {
					...
					kalMemCopy(&(prAdapter->rWlanInfo.arScanResult[0]),
						   ...
						   OFFSET_OF(PARAM_BSSID_EX_T, pucIE));
				}
...
WLAN_STATUS wlanLoadManufactureData(IN P_ADAPTER_T prAdapter, IN P_REG_INFO_T prRegInfo)
{
	...
	*/
	...
	if (prRegInfo->fg2G4BandEdgePwrUsed) {
		CMD_EDGE_TXPWR_LIMIT_T rCmdEdgeTxPwrLimit = {0};
...
* @param prAdapter      Pointer of Adapter Data Structure
*
* @return WLAN_ACPI_STATE_D0 Normal Operation Mode
*         WLAN_ACPI_STATE_D3 Suspend Mode
*/
/*----------------------------------------------------------------------------*/
...
*
* @param prAdapter      Pointer of Adapter Data Structure
* @param ePowerState    WLAN_ACPI_STATE_D0 Normal Operation Mode
*                       WLAN_ACPI_STATE_D3 Suspend Mode
*
* @return none
*/
...
{
	ASSERT(prAdapter);
	ASSERT(ePowerState <= WLAN_ACPI_STATE_D3);
					...
					IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen, BOOLEAN fgIsOid)
{
	...
	CMD_GET_STA_STATISTICS_T rQueryCmdStaStatistics = {0};
	UINT_8 ucIdx;
	ENUM_WMM_ACI_T eAci;
