...
/* NAN Opration Mode Field */
#define NAN_DEV_CAP_OP_MODE_PHY_MODE_VHT BIT(0)
#define NAN_DEV_CAP_OP_MODE_PHY_MODE_BW_80P80 BIT(1)
#define NAN_DEV_CAP_OP_MODE_PHY_MODE_BW_160 BIT(2)
...
struct _NAN_ATTR_NDPE_T {
	...
	uint8_t ucPublishID;	/* optional */
	uint8_t aucResponderNDI[6]; /*optional */
	uint8_t aucTLVList[];      /*to be defined*/
} __KAL_ATTRIB_PACKED__;
...
struct _NAN_ATTR_CIPHER_SUITE_INFO_T {
	...
	uint16_t u2Length;
	uint8_t ucCapabilities;
	uint8_t aucCipherSuiteList[]; /* NAN_CIPHER_SUITE_ATTRIBUTE_T */
} __KAL_ATTRIB_PACKED__;
