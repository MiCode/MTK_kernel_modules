		...
		if (prAdapter->fgIsSupportMailBoxDriverOwnAck &&
			...
			if (fgResult) {
				...
				halGetMailbox(prAdapter,
					ENUM_SDIO_MAILBOX_DRIVER_OWN_ACK,
					&u4Value);
#else
...
*/
/*----------------------------------------------------------------------------*/
static u_int8_t halClearMailBoxAck(IN struct ADAPTER *prAdapter)
{
	u_int8_t fgStatus = TRUE;
	uint32_t u4Value = 0;
...
void halSetFWOwn(IN struct ADAPTER *prAdapter, IN u_int8_t fgEnableGlobalInt)
{
	...
	if (kalIsRstPreventFwOwn(prAdapter->prGlueInfo) == TRUE) {
...
{
	glSetRstPreventFwOwn(NULL, fgEnable);
	DBGLOG(INIT, STATE, "fgIsRstPreventFwOwn = %d", fgEnable);
...
#endif
	prChipInfo = prAdapter->chip_info;
	glSetRstPreventFwOwn(prAdapter->prGlueInfo, FALSE);
