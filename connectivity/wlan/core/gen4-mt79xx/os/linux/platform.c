...
#if (CFG_SUPPORT_MULTI_CARD == 0)       /* unused  */
...
void wlanRegisterInetAddrNotifier(struct GLUE_INFO *prGlueInfo)
{
	...
	struct notifier_block *prNotifier = &prGlueInfo->inetaddr_notifier;

	prNotifier->notifier_call = netdev_event;
#else
	struct notifier_block *prNotifier = &inetaddr_notifier;
#endif

#if CFG_ENABLE_NET_DEV_NOTIFY

	register_inetaddr_notifier(prNotifier);
#if 0				/* CFG_SUPPORT_PASSPOINT */
...
void wlanUnregisterInetAddrNotifier(struct GLUE_INFO *prGlueInfo)
{
	...
	unregister_inetaddr_notifier(prNotifier);
#if 0				/* CFG_SUPPORT_PASSPOINT */
...
int wlan_netdev_notifier_call(struct notifier_block *nb,
		unsigned long state, void *ndev)
{
...
{
#if CFG_SUPPORT_MULTI_CARD
	struct notifier_block *prNotifier = &prGlueInfo->wlan_netdev_notifier;

	prNotifier->notifier_call = wlan_netdev_notifier_call;
#else
	struct notifier_block *prNotifier = &wlan_netdev_notifier;
#endif
