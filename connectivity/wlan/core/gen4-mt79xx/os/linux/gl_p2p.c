...
struct wireless_dev *wlanGetP2pWdev(struct GLUE_INFO *prGlueInfo)
{
#if CFG_SUPPORT_MULTI_CARD
	if (!prGlueInfo)
		return NULL;

	return prGlueInfo->prP2pWdev;
#else
	return gprP2pWdev;
#endif
}

struct wireless_dev **wlanGetP2pRoleWdev(struct GLUE_INFO *prGlueInfo)
{
#if CFG_SUPPORT_MULTI_CARD
	if (!prGlueInfo)
		return NULL;

	return prGlueInfo->prP2pRoleWdev;
#else
	return gprP2pRoleWdev;
#endif
}
...
#endif
#if (CFG_SUPPORT_DFS_MASTER == 1)
			if (prGlueInfo->prP2PInfo[ucIdex]->chandef == NULL) {
				prGlueInfo->prP2PInfo[ucIdex]->chandef =
					(struct cfg80211_chan_def *)kalMemAlloc(
					sizeof(struct cfg80211_chan_def),
					VIR_MEM_TYPE);
				if (prGlueInfo->prP2PInfo[ucIdex]->chandef
					== NULL)
					goto p2p_alloc_fail;
				...
				prGlueInfo->prP2PInfo[ucIdex]->chandef->chan =
					(struct ieee80211_channel *)kalMemAlloc(
...
p2p_alloc_fail:
	...
	if (prGlueInfo->prP2PInfo[ucIdex]->chandef) {
		if (prGlueInfo->prP2PInfo[ucIdex]->chandef->chan) {
			kalMemFree(prGlueInfo->prP2PInfo[ucIdex]
				->chandef->chan, VIR_MEM_TYPE,
				sizeof(struct ieee80211_channel));
			prGlueInfo->prP2PInfo[ucIdex]
				->chandef->chan = NULL;
		}
		kalMemFree(prGlueInfo->prP2PInfo[ucIdex]->chandef,
			VIR_MEM_TYPE, sizeof(struct cfg80211_chan_def));
		prGlueInfo->prP2PInfo[ucIdex]->chandef = NULL;
	}
...
u_int8_t p2PFreeInfo(struct GLUE_INFO *prGlueInfo, uint8_t ucIdx)
{
	...
	if (prGlueInfo->prP2PInfo[ucIdx] != NULL) {
		...
		prGlueInfo->prP2PInfo[ucIdx]->pP2pStartAPMsg = NULL;
		...
		if (prGlueInfo->prP2PInfo[ucIdx]->chandef) {
			...
			kalMemFree(prGlueInfo->prP2PInfo[ucIdx]->chandef,
				VIR_MEM_TYPE, sizeof(struct cfg80211_chan_def));
		...
		u_int8_t fgIsRtnlLockAcquired)
{
...
#if CFG_SUPPORT_MULTI_CARD
	prDevIdx = prGlueInfo->u4P2pDevIdx;
#else
	prDevIdx = g_u4DevIdx;
#endif
	pprP2pDev = wlanGetP2pNetDev(prGlueInfo);
	...
	if (register_netdev(prGlueInfo->prP2PInfo[0]->prDevHandler) < 0) {
		...
		pprP2pDev[0] = prGlueInfo->prP2PInfo[0]->prDevHandler;
		ret = TRUE;
	}
	...
	if (prGlueInfo->prAdapter->prP2pInfo->u4DeviceNum == KAL_P2P_NUM) {
		...
		if (register_netdev(
			...
			prGlueInfo->prAdapter->rP2PNetRegState =
				ENUM_NET_REG_STATE_REGISTERED;
			pprP2pDev[1] = prGlueInfo->prP2PInfo[1]->prDevHandler;
			ret = TRUE;
		}
		...
		const char *prDevName2, uint8_t ucApMode)
{
	...
	do {
		...
		pprP2pWdev = wlanGetP2pRoleWdev(prGlueInfo);
...
u_int8_t glP2pCreateWirelessDevice(struct GLUE_INFO *prGlueInfo)
{
	...
	prWiphy = wlanGetWiphy(pprWdev[0]);
...
void mtk_p2p_wext_set_Multicastlist(struct GLUE_INFO *prGlueInfo)
{
	...
	uint32_t u4McCount;
#if CFG_SUPPORT_MULTI_CARD
	struct net_device *prDev = prGlueInfo->p2pPrDev;
#else
