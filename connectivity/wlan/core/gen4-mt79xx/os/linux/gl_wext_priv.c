	...
	IN int i4TotalLen)
{
	...
	uint8_t ucCount = 0;
	uint32_t au4BandWidthTable[] = {120, 130, 140, 150, 160, 320};
	...
	case ENUM_RADAR_CTRL_SET_RADAR:
		...
		prRadarSetting->u2CTSNAV = 15000;
		prRadarSetting->ucCBPEnable = 2;
		...
		if (rStatus || ((u4Freq != 5800000) && (u4Freq != 6095000))) {
...
cmd_invalid:
	...
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\nHelp menu\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\tset Radar Params:\t\"SET_RADAR_PARAMS 0 [Freq] [BandWidth] [CTSNAV] [CBPEnable]\"\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\t[Freq]: 5800000, 6095000\"\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\t[BandWidth]: 150\"\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\t[CTSNAV]: 0 ~ 32767\"\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\t[CBPEnable]: 0(off), 1(on), 2(5G off, 6G on)\"\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\tenable Radar\t\"SET_RADAR_PARAMS 1 [Enable:1\\Disable:0] [FPS]\"\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\t[FPS]: 1 ~ 10\"\n");
			...
			int i4TotalLen)
{
	...
	uint8_t ucMode = PWR_MODE_6G_NUM, c = 0;
	 ...
	 * iwpriv wlan0 driver
	 *        "SET_6G_PWR_MODE 6GPwrModeEnable6VL [num]"
	...
	for (c = 0; (c < i4Argc) && (c + 1 < i4Argc); c++) {
		if (kalStrCmp(apcArgv[c], "mode") == 0) {
			...
			if (rStatus != WLAN_STATUS_SUCCESS)
				return WLAN_STATUS_FAILURE;
		} else if (kalStrCmp(apcArgv[c],
			"6GPwrModeEnable6XD") == 0) {
			...
			prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6XD]
					= ucEnable;
		} else if (kalStrCmp(apcArgv[c],
			"6GPwrModeEnable6FX") == 0) {
			u4Ret = kalkStrtou8(apcArgv[c+1], 0, &ucEnable);
			if (u4Ret) {
				DBGLOG(REQ, ERROR,
					"parse set_6g_pwr_mode error Ret=%d\n",
			...
			prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6FX]
					= ucEnable;
		} else if (kalStrCmp(apcArgv[c],
			"6GPwrModeEnable6CD") == 0) {
			u4Ret = kalkStrtou8(apcArgv[c+1], 0, &ucEnable);
			if (u4Ret) {
				DBGLOG(REQ, ERROR,
					"parse set_6g_pwr_mode error Ret=%d\n",
			...
			prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6CD]
					= ucEnable;
		} else if (kalStrCmp(apcArgv[c],
			"6GPwrModeEnable6VL") == 0) {
			u4Ret = kalkStrtou8(apcArgv[c+1], 0, &ucEnable);
			if (u4Ret) {
				DBGLOG(REQ, ERROR,
					"parse set_6g_pwr_mode error Ret=%d\n",
			...
			prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6VL]
					= ucEnable;
	...
	i4BytesWritten += kalScnprintf(pcCommand + i4BytesWritten,
	i4TotalLen - i4BytesWritten,
	"\nPwrMode[%d] 6XD/6FX/6CD/6VL/Num[%u %u %u %u %d] Force[%u]\n",
		prAdapter->e6GPwrModeCurr,
		prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6XD],
		prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6FX],
		prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6CD],
		prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6VL],
		PWR_MODE_6G_RULE_NUM,
