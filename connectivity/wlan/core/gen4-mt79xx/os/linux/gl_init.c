...
static void wlanFreeNetDev(struct net_device *prNetDev)
{
	...
	for (u4Idx = 0; u4Idx < KAL_AIS_NUM; u4Idx++) {
		...
		netName = pprWdev[u4Idx]->netdev->name;

		DBGLOG(INIT, INFO, "free_netdev %s netdev start.\n", netName);
		free_netdev(pprWdev[u4Idx]->netdev);
		DBGLOG(INIT, INFO, "free_netdev %s netdev end.\n", netName);
	}
}
...
static void wlanTrackingProbeRemoveClear(u_int8_t fgIsProbe, uint32_t bus_id)
{
    ...
    unsigned long flags;
    struct probe_remove_tracking_item *cur, *next;
    struct probe_remove_tracking_item *to_free = NULL;
    ...
    if (!list_empty(pList)) {
        list_for_each_entry_safe(cur, next, pList, node) {
            if (cur->bus_id == bus_id) {
                fgRet = TRUE;
                list_del(&cur->node);
                to_free = cur; // Mark the item for deletion outside the spinlock
                break; // Assuming only one match, break after the first find
            }
        }
    }
    spin_unlock_irqrestore(&g_probe_remove_lock, flags);
    ...
    if (to_free) {
        kalMemFree(to_free, VIR_MEM_TYPE,
		   sizeof(struct probe_remove_tracking_item));
    }
...
static void exitWlan(void)
{
	...
	glUnregisterBus(wlanRemove, (void *) prDev);
#else
		...
		unsigned long event, void *unused)
{
	...
	if (event == SYS_RESTART) {
		...
		prGlueInfo = container_of(nb,
					struct GLUE_INFO, wlan_fb_notifier);
#else
...
#if defined(_HIF_USB) || defined(_HIF_PCIE)
		glUnregisterBus(wlanRemove, (void *) prGlueInfo->prDev);
#else
		glUnregisterBus(wlanRemove);
#endif
