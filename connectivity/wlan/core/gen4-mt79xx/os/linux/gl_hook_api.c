...
int32_t MT_ATE_listmode_cmd(struct net_device *prNetDev,
							void *SegHeader,
							uint32_t cmd_type)
{
	uint32_t u4BufLen = 0;
	int32_t i4Status = 0;
	struct GLUE_INFO *prGlueInfo = NULL;
	struct list_mode_tx_seg_header *TxParserSegHeader = NULL;
	struct list_mode_rx_seg_header *RxParserSegHeader = NULL;
	uint32_t header_size = 0;

	if (cmd_type == LISTMODE_RX_SEG) {
		RxParserSegHeader = SegHeader;
		header_size = sizeof(struct list_mode_rx_seg_header) +
		LIST_MODE_FW_SEG_NUM_MAX * sizeof(uint32_t) *
		RxParserSegHeader->u4SegParaNum;
	} else if (cmd_type == LISTMODE_TX_SEG) {
		TxParserSegHeader = SegHeader;
		header_size = sizeof(struct list_mode_tx_seg_header) +
		LIST_MODE_FW_SEG_NUM_MAX * sizeof(uint32_t) *
		TxParserSegHeader->u4SegParaNum;
	} else {
		return -EFAULT;
		/*listmode cmd type ERROR!*/
	}

	prGlueInfo = *((struct GLUE_INFO **) netdev_priv(prNetDev));
	i4Status = 0;
	if (cmd_type == LISTMODE_RX_SEG) {
		wlanoidListMode(prGlueInfo->prAdapter,
			 RxParserSegHeader, header_size,
			 &u4BufLen);
	} else {
		wlanoidListMode(prGlueInfo->prAdapter,
			 TxParserSegHeader, header_size,
			 &u4BufLen);
	}
...
int32_t MT_ATE_listmode_rx_get_status(struct net_device *prNetDev,
			struct list_mode_rx_get_status *rx_status,
			struct list_mode_event *rx_event)
{
	uint32_t u4BufLen = 0;
	int32_t i4Status = 0;
	struct GLUE_INFO *prGlueInfo = NULL;
	char *ParserSegHeader = NULL;
	uint32_t header_size = 0;

	ParserSegHeader = (char *)rx_status;
	header_size = sizeof(struct list_mode_rx_get_status);

	prGlueInfo = *((struct GLUE_INFO **) netdev_priv(prNetDev));

	i4Status = 0;
	wlanoidListMode(prGlueInfo->prAdapter,
			 ParserSegHeader, header_size,
			 &u4BufLen);
	kalMemCopy(rx_event,
				&g_HqaListModeStatus,
...
#endif //CFG_CONNAC2_LISTMODE_ENABLE == 1
	 ...
	 void *rsp_data)
{
	...
	} else if (oidType == OP_WLAN_OID_LIST_MODE) {
		DBGLOG_MEM8(RFTEST,
				TRACE,
				&g_HqaListModeStatus,
				sizeof(g_HqaListModeStatus));
...
int glRFTestL0P5(void *data)
{
	...
	/* check normal fw bin is support TestMode or not */
	if (!prGlueInfo->fgSupportTestMode) {
		/* Flush any pending reset operations */
	...
	/* check test fw bin is support TestMode or not*/
	if (!prGlueInfo->fgSupportTestMode) {
		DBGLOG(RFTEST, ERROR,
			"Test FW bin not suuport Test Mode\n");
		return u4Status;
	}
