...
struct list_mode_tx_seg_header {
	...
	uint32_t	u4SeqID;
	uint32_t	u4TxLen;
	uint8_t	aucSourceAddr[SERV_MAC_ADDR_LEN];
	uint8_t	aucDestAddr[SERV_MAC_ADDR_LEN];
	uint8_t	aucBSSID[SERV_MAC_ADDR_LEN];
	uint32_t	u4STBC;
	uint32_t	u4SegNum;
	uint32_t	u4SegParaNum;
...
struct list_mode_rx_seg_header {
	uint32_t	u4ExtId;
	uint8_t	aucOwnMac[SERV_MAC_ADDR_LEN];
	uint32_t	u4SegNum;
	uint32_t	u4SegParaNum;
...
#define LIST_MODE_FW_SEG_NUM_MAX	12
...
uint32_t glSetRFTestMode(struct GLUE_INFO *prGlueInfo,
	u_int8_t fgEn);
