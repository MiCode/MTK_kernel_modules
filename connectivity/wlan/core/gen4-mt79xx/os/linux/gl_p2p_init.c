...
u_int8_t p2pRemove(struct GLUE_INFO *prGlueInfo)
{
	...
	struct wireless_dev *prP2pWdev = wlanGetP2pWdev(prGlueInfo);
	...
	for (idx = 0 ; idx < KAL_P2P_NUM; idx++) {
		if (pprP2pRoleWdev[idx] == NULL)
			continue;
#if CFG_ENABLE_UNIFY_WIPHY
		if (wlanIsAisDev(pprP2pRoleWdev[idx]->netdev)) {
			/* This is AIS/AP Interface */
			pprP2pRoleWdev[idx] = NULL;
			continue;
		}
#endif
		/* free gprP2pWdev in wlanDestroyAllWdev */
		if (pprP2pRoleWdev[idx] == prP2pWdev)
			continue;

		DBGLOG(INIT, INFO, "Unregister pprP2pRoleWdev[%d]\n", idx);
#if (CFG_ENABLE_UNIFY_WIPHY == 0)
		set_wiphy_dev(pprP2pRoleWdev[idx]->wiphy, NULL);
		wiphy_unregister(pprP2pRoleWdev[idx]->wiphy);
		wiphy_free(pprP2pRoleWdev[idx]->wiphy);
#endif
		kfree(pprP2pRoleWdev[idx]);
		pprP2pRoleWdev[idx] = NULL;
		break;
	}
#if (CFG_ENABLE_UNIFY_WIPHY == 0)
	 ...
	 */
	if (prP2pWdev != NULL)
		set_wiphy_dev(prP2pWdev->wiphy, NULL);
#endif
