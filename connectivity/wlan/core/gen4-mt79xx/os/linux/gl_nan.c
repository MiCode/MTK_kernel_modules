 ...
 */
/*----------------------------------------------------------------------------*/
uint32_t nanDeinitAtReset(struct GLUE_INFO *prGlueInfo)
{
	...
	/* do NAN reset */
	if (kalIsResetting(prGlueInfo) == FALSE) {
		...
		rWlanStatus = (uint32_t)set_nan_handler(prNetDev, FALSE);
		nanSetAtResetFlow(FALSE);
	}

	if (rWlanStatus == WLAN_STATUS_SUCCESS)
		DBGLOG(INIT, ERROR, "NAN reset success\n");
	else
		DBGLOG(INIT, ERROR, "NAN reset fail: 0x%08x\n", rWlanStatus);
	   ...
	   struct net_device *prNanDev, uint8_t ucRoleIdx)
{
	...
	for (ucIdx = 0; ucIdx < NAN_BSS_INDEX_NUM; ucIdx++) {
		...
		ucBssIndex = nanDevInit(prGlueInfo->prAdapter, ucIdx);

		if (ucBssIndex == MAX_BSS_INDEX) {
			DBGLOG(INIT, ERROR, "No BSS can be used!!\n");
			nanFreeInfo(prGlueInfo, ucIdx);
			return -1;
		}
