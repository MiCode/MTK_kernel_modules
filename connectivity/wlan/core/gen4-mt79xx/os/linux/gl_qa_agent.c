			       ...
			       struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memcpy(HqaCmdFrame->Data + 4 * 1, rRfATInfo.u4FuncFmcwData,
		       sizeof(rRfATInfo.u4FuncFmcwData));

	ResponseToQA(HqaCmdFrame, prIwReqData, 2 + sizeof(rRfATInfo), i4Ret);
...
static int32_t hqa_listmode_tx_cmd(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (pbuf == NULL)
		goto leave_hqa_listmode_tx_cmd;
...
static int32_t hqa_listmode_rx_cmd(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (pbuf == NULL)
		goto leave_hqa_listmode_rx_cmd;
...
static HQA_CMD_HANDLER hqa_ext_cmd_set[] = {
	...
	hqa_listmode_tx_cmd,		/* 0x00000011 */
