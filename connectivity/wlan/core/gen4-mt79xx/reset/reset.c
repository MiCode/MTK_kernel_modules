...
void resetSetState(enum ENUM_RST_STATE_TYPE_T eNextstate)
{
	...
	if (((unsigned int)g_prResetInfo.eResetState >= RST_STATE_MAX) ||
	    ((unsigned int)eNextstate >= RST_STATE_MAX))
		return;
	MR_Dbg("%s: current_state[%s], next_state[%s]\n", __func__,
		apcState[(unsigned int)g_prResetInfo.eResetState],
		apcState[(unsigned int)eNextstate]);
				...
				void *data)
{
	...
	MR_Dbg("%s: module[%s], status[%s]\n", __func__,
		apcModule[(unsigned int)module],
		apcStatus[(unsigned int)status]);
