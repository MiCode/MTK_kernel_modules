...
#define TX_PWR_ANT_GAIN_GROUP_NUM	(4)
...
#endif  /*CFG_SUPPORT_TX_PWR_ENV == 1*/
	...
	enum ENUM_PWR_MODE_6G_TYPE e6GPwrModeBss)
{
	...
	if (e6GPwrModeCurr != e6GPwrModeFinal) {
		/* Resend power limit */
		prAdapter->e6GPwrModeCurr = e6GPwrModeFinal;
		rlmDomainUpdatePwrLimit_6G_By_PowerMode(prAdapter,
			e6GPwrModeFinal);
	} else if (prAdapter->e6GPwrModeCurr != e6GPwrModeFinal) {
		/* Resend power limit */
		DBGLOG(RLM, INFO, "Set to PwrMode[%d]Curr[%d]",
			e6GPwrModeFinal,
	...
	DBGLOG(RLM, INFO, "Update BSS[%u]6GPwrMode[%d]Curr[%d]Final[%d]",
			ucBssIndex,
			e6GPwrModeBss,
			e6GPwrModeCurr,
			e6GPwrModeFinal);
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief This func is use to decide the supported 6G power mode for STA
 *
 * \param[in] prAdapter : Pointer to adapter
 * \param[in] ucChnlNum : Channel Number
 * \param[in] e6GPwrModeCurr : Enum of 6G power mode
 ...
 */
/*----------------------------------------------------------------------------*/
uint8_t rlmDomainSupported6GPwrMode(
	struct ADAPTER *prAdapter,
	uint8_t ucChnlNum,
	...
	uint8_t *pfgSupport)
{
	...
	uint8_t *str = NULL;
	uint32_t u4ChnlFlag = 0;
#if (CFG_SUPPORT_SINGLE_SKU == 1)
	uint8_t i = 0, max_count = 0;
	struct CMD_DOMAIN_CHANNEL *prCh = NULL;
	...
	for (; i < max_count; i++) {
		prCh = RLM_GET_ACTIVE_CHANNELS() + i;
		if (prCh->u2ChNum == ucChnlNum) {
			u4ChnlFlag = prCh->eFlags;
			break;
		}
	}

	if (i == max_count) {
		ePwrMode6G = e6GPwrModeCurr;
		*pfgSupport = FALSE;
		DBGLOG(RLM, WARN, "Mismatch with ChnlNum[%u]6GPwrMode[%d]",
			ucChnlNum, ePwrMode6G);
		return ePwrMode6G;
	}
#endif /* CFG_SUPPORT_SINGLE_SKU */

	if ((prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6CD]) &&
		(e6GPwrModeCurr <= PWR_MODE_6G_LPI)) {
		/* follow ap setting when LPI and SP */
		ePwrMode6G = e6GPwrModeCurr;
		str = "6CD";
		if ((ePwrMode6G == PWR_MODE_6G_SP) &&
			((u4ChnlFlag & IEEE80211_CHAN_NO_6GHZ_AFC_CLIENT)
				== IEEE80211_CHAN_NO_6GHZ_AFC_CLIENT))
			*pfgSupport = FALSE;
		else
			*pfgSupport = TRUE;
	} else if ((prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6XD]) &&
		(e6GPwrModeCurr == PWR_MODE_6G_LPI)) {
		/* Use LPI oply */
		ePwrMode6G = PWR_MODE_6G_LPI;
		str = "6XD";
		*pfgSupport = TRUE;
	} else if ((prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6FX]) &&
		(e6GPwrModeCurr == PWR_MODE_6G_SP)) {
		/* Use SP only */
		ePwrMode6G = PWR_MODE_6G_SP;
		str = "6FX";
		if ((u4ChnlFlag & IEEE80211_CHAN_NO_6GHZ_AFC_CLIENT)
			== IEEE80211_CHAN_NO_6GHZ_AFC_CLIENT)
			*pfgSupport = FALSE;
		else
			*pfgSupport = TRUE;
	} else if (prWifiVar->auc6GPwrModeRuleEn[PWR_MODE_6VL]) {
		/* Use VLP only or Try to use VLP */
		ePwrMode6G = PWR_MODE_6G_VLP;
		str = "6VL";
		if ((u4ChnlFlag & IEEE80211_CHAN_NO_6GHZ_VLP_CLIENT)
				== IEEE80211_CHAN_NO_6GHZ_VLP_CLIENT)
			*pfgSupport = FALSE;
		else
			*pfgSupport = TRUE;
	} else {
		/* Need to drop beacon when there is no matched rule,
		 * Assign e6GPwrModeCurr to print supported 6G PwrMode
		 * for AP in log
		 */
		ePwrMode6G = e6GPwrModeCurr;
		str = "None";
	...
	DBGLOG(RLM, TRACE,
		"Rule[%s/%u/0x%x]PwrMode[%d->%d]Support[%u]\n",
		str,
		ucChnlNum,
		u4ChnlFlag,
		e6GPwrModeCurr,
	...
	enum ENUM_PWR_MODE_6G_TYPE ePwrMode6G)
{
	...
	if (ePwrMode6G == PWR_MODE_6G_VLP)
		prAdapter->chip_info->prTxPwrLimitFile =
		(char *)"TxPwrLimit6G_MT79x1_VLP.dat";
	else if (ePwrMode6G == PWR_MODE_6G_SP)
		prAdapter->chip_info->prTxPwrLimitFile =
		(char *)"TxPwrLimit6G_MT79x1_SP.dat";
	else
		prAdapter->chip_info->prTxPwrLimitFile =
		(char *)"TxPwrLimit6G_MT79x1.dat";
...
#endif /* #if (CFG_SUPPORT_WIFI_6G_PWR_MODE == 1) */

#if (CFG_SUPPORT_TX_PWR_ENV == 1)
uint32_t rlmTxPwrEnvMaxPwrSend(
	struct ADAPTER *prAdapter,
	enum ENUM_BAND eBand,
	uint8_t ucPriCh,
	uint8_t ucPwrLmtNum,
	...
	uint8_t fgPwrLmtEnable)
{
	...
	kalMemSet(prTempCmd->u.rTxPowerEnvLimit.acMaxTxPwrLmt,
		MAX_TX_POWER,
		sizeof(prTempCmd->u.rTxPowerEnvLimit.acMaxTxPwrLmt));

	/*copy partial tx pwr limit*/
	prTempCmd->ucNum = 1;
	prTempCmd->eBand = (uint8_t)eBand;
	prTempCmd->eLimitType = TXPOWER_LIMIT_FORMAT_TX_PWR_ENV;
	prTempCmd->bCmdFinished = true;
	prTempCmd->u.rTxPowerEnvLimit.fgPwrLmtEnable = fgPwrLmtEnable;
	prTempCmd->u.rTxPowerEnvLimit.ucBand = (uint8_t)eBand;
	prTempCmd->u.rTxPowerEnvLimit.ucPriCh = ucPriCh;
	prTempCmd->u.rTxPowerEnvLimit.ucPwrLmtNum = ucPwrLmtNum;
	...
	for (eBwType = TX_PWR_ENV_MAX_TXPWR_BW20;
		...
		prTempCmd->
			u.rTxPowerEnvLimit.acMaxTxPwrLmt[ANTENNA_WF0][eBwType]
			= pcTxPwrEnvMaxPwr[eBwType] - acAntGain[ANTENNA_WF0];
		prTempCmd->
			u.rTxPowerEnvLimit.acMaxTxPwrLmt[ANTENNA_WF1][eBwType]
			= pcTxPwrEnvMaxPwr[eBwType] - acAntGain[ANTENNA_WF1];
	}

	DBGLOG(RLM, INFO, "[%d]Send_TPE, En[%u]Band[%u]Ch[%u]Num[%u]\n",
		__LINE__,
		prTempCmd->u.rTxPowerEnvLimit.fgPwrLmtEnable,
		prTempCmd->u.rTxPowerEnvLimit.ucBand,
		prTempCmd->u.rTxPowerEnvLimit.ucPriCh,
		prTempCmd->u.rTxPowerEnvLimit.ucPwrLmtNum);
...
#endif // #if (CFG_SUPPORT_TX_PWR_ENV == 1)
