...
#elif (CFG_SUPPORT_WIFI_6G_PWR_MODE == 1)
	struct WIFI_VAR *prWifiVar;
...
uint8_t
p2pFuncSelect5GNonDFSChannel(struct ADAPTER *prAdapter, uint8_t ucBandWidth)
{
	uint8_t ucNumOfChannel = 0;
	uint8_t ucNewChannel = 0;
	u_int8_t ch_idx = 0;
	struct RF_CHANNEL_INFO aucChannelList[MAX_5G_BAND_CHN_NUM] = {0};
	struct RF_CHANNEL_INFO aucChannelListRdd[MAX_5G_BAND_CHN_NUM] = {0};
	...
	p2pFuncChannelListFiltering(prAdapter, ucBandWidth, ucNumOfChannel,
			aucChannelList, &ucNumOfChannel, aucChannelListRdd);
