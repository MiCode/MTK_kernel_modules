	...
	uint8_t ucBssIndex, struct STA_RECORD *prStaRec)
{
	...
	if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.
		auc6GPwrModeRuleEn[PWR_MODE_6VL]) &&
		RLM_NET_IS_11AX(prBssInfo) &&
		(ucPhyTypeSet & PHY_TYPE_SET_802_11AX) &&
		prBssInfo->eBand == BAND_6G)
		return ELEM_HDR_LEN + sizeof(struct IE_TX_PWR_ENV_FRAME);
#endif
	...
	struct MSDU_INFO *prMsduInfo)
{
	...
	if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.
		...
		pucBuffer =
			(uint8_t *) ((unsigned long)prMsduInfo->prPacket +
			(unsigned long)prMsduInfo->u2FrameLength);
		/* Fill Tx Pwr Envelope IE with VLP */
		TPE_IE(pucBuffer)->ucId = ELEM_ID_TPE;
		TPE_IE(pucBuffer)->ucLength = 2;
		TPE_IE(pucBuffer)->ucTxPwrInfo = 0x18;
		TPE_IE(pucBuffer)->acMaxTxPwr[0] = 0xea;
...
#if 0 //(CFG_SUPPORT_WIFI_6G_PWR_MODE == 1)
		/* Since TPE IE already minus 6dB from AP, client device doesn't
		 * need to minus 6dB again
		 */
		if (eHwBand == BAND_6G) {
