...
void aisFsmInit(IN struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	cnmTimerInitTimer(prAdapter,
		&prAisFsmInfo->rBeaconLossTimer,
		(PFN_MGMT_TIMEOUT_FUNC) aisFsmRunEventBeaconLossTimeOut,
		(unsigned long)ucBssIndex,
		TIMER_WAKELOCK_AUTO);
	cnmTimerInitTimer(prAdapter,
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
	cnmTimerStopTimer(prAdapter, &prAisFsmInfo->rBeaconLossTimer);
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
			DBGLOG(AIS, INFO,
				"stop beacon loss timer at IDLE state\n");
			cnmTimerStopTimer(prAdapter,
				&prAisFsmInfo->rBeaconLossTimer);
#endif
			break;
				...
				IN struct MSG_HDR *prMsgHdr)
{
	...
	if (eNextState == AIS_STATE_NORMAL_TR) {
		DBGLOG(AIS, INFO,
			"stop beacon loss timer when reconnect success.\n");
		cnmTimerStopTimer(prAdapter, &prAisFsmInfo->rBeaconLossTimer);
		DBGLOG(AIS, INFO,
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
void aisFsmRunEventBeaconLossTimeOut(IN struct ADAPTER *prAdapter,
				  unsigned long ulParamPtr)
{
	uint8_t ucBssIndex = (uint8_t)ulParamPtr;
	struct BSS_INFO *prBssInfo = NULL;

	DBGLOG(AIS, INFO, "Beacon Loss Timer Timeout\n");
