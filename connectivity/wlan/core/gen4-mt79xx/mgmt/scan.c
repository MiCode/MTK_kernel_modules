				  ...
				  IN struct SW_RFB *prSwRfb)
{
	...
	if ((eHwBand == BAND_6G) && ((prAdapter->fg6GPwrModeForce) != TRUE)) {
		...
		e6GPwrModeCurr = rlmDomain6GPwrModeDecision(
					...
					fgIsHE6GPresent,
					uc6GHeRegInfo);
		e6GPwrModeCurr = rlmDomainSupported6GPwrMode(
					prAdapter,
					ucChnlNum,
					e6GPwrModeCurr,
					&fgPwrMode6GSupport);
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_MBSSID:
			if (MBSSID_IE(pucIE)->ucLength < 2) {
				DBGLOG(SCN, WARN, "MBSSID IE_LEN err(%u)!\n",
	...
	if (prTxPwrEnvIE) {
		...
		DBGLOG_MEM8(SCN, INFO, prTxPwrEnvIE, IE_SIZE(prTxPwrEnvIE));
...
void scanCheckEpigramVhtIE(IN uint8_t *pucBuf, IN struct BSS_DESC *prBssDesc)
{
	...
	if (prEpiIE->ucLength < OFFSET_OF(
		struct IE_VENDOR_EPIGRAM_IE, pucData[0])) {
		DBGLOG(RLM, WARN,
			"[Epigram] prEpiIE->ucLength is too short, skip!\n");
		return;
	}
