			   ...
			   OUT uint16_t *pu2StatusCode)
{
	...
	*pu2StatusCode = prAuthFrame->u2StatusCode;
	...
	if (*pu2StatusCode == STATUS_CODE_ASSOC_REJECTED_TEMPORARILY &&
		prAdapter->prGlueInfo->u4RoamingEnhanceEnable) {
		aisGetAisSpecBssInfo(prAdapter,
			prStaRec->ucBssIndex)->fgIgnorSaQueryReq = TRUE;
		cnmTimerStartTimer(prAdapter,
			&aisGetAisFsmInfo(
				prAdapter,
				prStaRec->ucBssIndex)->rPmfWaitReconnectTimer,
			TU_TO_MSEC(PMF_DEFAULT_ASSOC_COMEBACK) +
		    ...
		    IN PFN_TX_DONE_HANDLER pfTxDoneHandler)
{
	...
	u2EstimatedFrameLen =
	    (MAC_TX_RESERVED_FIELD + WLAN_MAC_MGMT_HEADER_LEN +
	     REASON_CODE_FIELD_LEN + ucExtra_length);
	...
	if (ucStaRecIdx == STA_REC_INDEX_BMCAST) {
		...
		prDeauthFrame = (struct WLAN_DEAUTH_FRAME *)(uint8_t *)
			    ((unsigned long)(prMsduInfo->prPacket)
			     + MAC_TX_RESERVED_FIELD);
		prDeauthFrame->aucInfoElem[0] = ELEM_ID_MMIC;
		prDeauthFrame->aucInfoElem[1] = MMIC_IE_LENGTH - 2;
		prDeauthFrame->aucInfoElem[2] = 4; /*igtk key id*/
		prDeauthFrame->aucInfoElem[3] = 0;
	}
#endif
