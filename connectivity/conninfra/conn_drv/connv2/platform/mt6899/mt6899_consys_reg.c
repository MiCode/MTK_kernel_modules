...
struct consys_base_addr conn_reg_mt6899;

static struct conn_debug_info_mt6899 static_debug_info;
static struct conn_debug_info_mt6899 *debug_info;
static char static_debug_buf[CONSYS_DUMP_BUF_SIZE];
static char *debug_buf;

static void consys_print_log(const char *title, struct conn_debug_info_mt6899 *info)
{
	char temp[13];
	int i;
...
static void consys_print_power_debug(enum conninfra_bus_error_type level)
{
	...
	consys_print_power_debug_dbg_level_0_mt6899_debug_gen(level, debug_info);
	consys_print_log("[CONN_POWER_A]", debug_info);

	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON) {
		consys_print_power_debug_dbg_level_1_mt6899_debug_gen(level, debug_info);
		consys_print_log("[CONN_POWER_B]", debug_info);
	}
	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF) {
		consys_print_power_debug_dbg_level_2_mt6899_debug_gen(level, debug_info);
...
static void consys_print_bus_debug(enum conninfra_bus_error_type level)
{
	...
	consys_print_bus_slpprot_debug_dbg_level_0_mt6899_debug_gen(level, debug_info);
	consys_print_log("[slpprot_a]", debug_info);

	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON) {
		consys_print_bus_debug_dbg_level_1_mt6899_debug_gen(level, debug_info);
		consys_print_log("[CONN_BUS_B]", debug_info);
	}
	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF) {
		consys_print_bus_debug_dbg_level_2_mt6899_debug_gen(level, debug_info);
		consys_print_log("[CONN_BUS_C]", debug_info);
		consys_print_bus_slpprot_debug_dbg_level_2_mt6899_debug_gen(level, debug_info);
...
int consys_check_conninfra_on_domain_status_mt6899(void)
{
	...
	/* - 0x1000_1C9C[0] / 0x1000_1C4C[25] (rx/tx) */
	...
	r2 = CONSYS_REG_READ_BIT(INFRACFG_AO_REG_BASE +
		CONSYS_GEN_INFRASYS_PROTECT_RDY_STA_0_OFFSET_ADDR, (0x1 << 25));
	if (r1 || r2) {
...
int consys_check_conninfra_bus_clock_status_mt6899(void)
{
	...
	while (count < 4) {
		r = consys_bus_hang_clock_detect_mt6899();
		if (r == 0x6)
			break;
...
void consys_debug_init_mt6899(void)
{
	...
	consys_debug_init_mt6899_debug_gen();
}
...
void consys_debug_deinit_mt6899(void)
{
	...
	consys_debug_deinit_mt6899_debug_gen();
}
