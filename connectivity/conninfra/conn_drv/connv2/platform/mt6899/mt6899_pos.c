...
#include "include/mt6899.h" /* For function declaration */
#include "include/mt6899_consys_reg.h" /* cr base address */
#include "include/mt6899_consys_reg_offset.h" /* cr offset */
 ...
 ********************************************************************************
 */
unsigned int mt6899_chip_info[CHIP_INFO_SIZE];
/*******************************************************************************
...
unsigned int consys_emi_set_remapping_reg_mt6899(
	phys_addr_t con_emi_base_addr,
	phys_addr_t md_shared_emi_base_addr,
	phys_addr_t gps_emi_base_addr)
{
	return consys_emi_set_remapping_reg_mt6899_gen(con_emi_base_addr, md_shared_emi_base_addr,
							gps_emi_base_addr, 16);
}

int consys_conninfra_wakeup_mt6899(void)
{
	return consys_conninfra_wakeup_mt6899_gen();
}

int consys_conninfra_sleep_mt6899(void)
{
	return consys_conninfra_sleep_mt6899_gen();
}

int consys_polling_chipid_mt6899(void)
{
	return consys_polling_chipid_mt6899_gen(NULL);
}

int connsys_d_die_cfg_mt6899(void)
{
	unsigned int d_die_efuse = 0;

	/* Reset conninfra sysram */
	consys_init_conninfra_sysram_mt6899_gen();

	/* Read D-die Efuse
	 * AP2CONN_EFUSE_DATA 0x1801_1020
	 * Write to conninfra sysram: CONN_INFRA_SYSRAM_SW_CR_D_DIE_EFUSE(0x1805_3820)
	 */
	connsys_get_d_die_efuse_mt6899_gen(&d_die_efuse);
	CONSYS_REG_WRITE(
		CONN_INFRA_SYSRAM_SW_CR_D_DIE_EFUSE, d_die_efuse);

	connsys_d_die_cfg_mt6899_gen();
					     ...
					     unsigned int next_status)
{
	...
	ret = connsys_adie_clock_buffer_setting_mt6899_gen(
...
int connsys_spi_master_cfg_mt6899(unsigned int curr_status, unsigned int next_status)
{
	if (curr_status != 0)
		return 0;

	connsys_wt_slp_top_ctrl_adie6637_mt6899_gen();
	return 0;
}

int consys_get_sleep_mode_mt6899(void)
{
	int platform_config;
...
int connsys_a_die_cfg_mt6899(unsigned int curr_status, unsigned int next_status)
{
	...
	int check = 0;
	struct consys_plat_thermal_data_mt6899 input;
	mapped_addr sysram_efuse_list_0[16] = { 0 };
	mapped_addr sysram_efuse_list_1[16] = { 0 };
	...
	clock_type = consys_co_clock_type_mt6899();
	...
	memset(&input, 0, sizeof(struct consys_plat_thermal_data_mt6899));

	connsys_a_die_switch_to_gpio_mode_mt6899_gen();
	connsys_adie_top_ck_en_ctl_mt6899_gen(1);
	connsys_a_die_cfg_deassert_adie_reset_mt6899_gen();
	...
	check = connsys_a_die_cfg_read_adie_id_mt6899_gen(&adie_id, &hw_ver_id);
	if (check) {
	...
	mt6899_chip_info[0] = adie_id;
	/* Write to conninfra sysram */
	CONSYS_REG_WRITE(CONN_INFRA_SYSRAM_SW_CR_A_DIE_CHIP_ID, adie_id);

	sysram_efuse_list_0[0] = (mapped_addr)CONN_INFRA_SYSRAM_SW_CR_A_DIE_EFUSE_DATA_0;
	sysram_efuse_list_0[1] = (mapped_addr)CONN_INFRA_SYSRAM_SW_CR_A_DIE_EFUSE_DATA_1;
	sysram_efuse_list_0[2] = (mapped_addr)CONN_INFRA_SYSRAM_SW_CR_A_DIE_EFUSE_DATA_2;
	sysram_efuse_list_0[3] = (mapped_addr)CONN_INFRA_SYSRAM_SW_CR_A_DIE_EFUSE_DATA_3;
	sysram_efuse_list_1[0] = (mapped_addr)CONN_INFRA_SYSRAM_SW_CR_A_DIE_EFUSE_DATA_4;
	sysram_efuse_list_1[1] = (mapped_addr)CONN_INFRA_SYSRAM_SW_CR_A_DIE_EFUSE_DATA_5;
	sysram_efuse_list_1[2] = (mapped_addr)CONN_INFRA_SYSRAM_SW_CR_A_DIE_EFUSE_DATA_6;
	sysram_efuse_list_1[3] = (mapped_addr)CONN_INFRA_SYSRAM_SW_CR_A_DIE_EFUSE_DATA_7;
	connsys_a_die_efuse_read_get_efuse_info_mt6899_gen(
			sysram_efuse_list_0, sysram_efuse_list_1,
			&(input.slop_molecule), &(input.thermal_b),&(input.offset));
	pr_info("slop_molecule=[%d], thermal_b =[%d], offset=[%d]",
		input.slop_molecule, input.thermal_b, input.offset);
	mt6899_chip_info[1] = CONSYS_REG_READ(sysram_efuse_list_1[2]);
	mt6899_chip_info[2] = CONSYS_REG_READ(sysram_efuse_list_1[3]);
	pr_info("Chip info = [%08x, %08x, %08x]",
		mt6899_chip_info[0], mt6899_chip_info[1], mt6899_chip_info[2]);
	update_thermal_data_mt6899(&input);

	connsys_a_die_cfg_PART2_mt6899_gen(hw_ver_id);

	consys_sema_release_mt6899(CONN_SEMA_RFSPI_INDEX);

	connsys_a_die_switch_to_conn_mode_mt6899_gen();

	conn_hw_env.is_rc_mode = consys_is_rc_mode_enable_mt6899();

	sleep_mode = consys_get_sleep_mode_mt6899();
	pr_info("sleep_mode = %d\n", sleep_mode);
	connsys_wt_slp_top_power_saving_ctrl_adie6637_mt6899_gen(adie_id, sleep_mode);
#endif /* CONFIG_FPGA_EARLY_PORTING */
	return 0;
}

void connsys_afe_sw_patch_mt6899(void)
{
	/* connsys_afe_sw_patch_mt6899_gen(); */
}

int connsys_afe_wbg_cal_mt6899(void)
{
	...
	if (first_cal == 0)
		return 0;
	first_cal = 0;
	return connsys_afe_wbg_cal_mt6899_gen(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT);
}

int connsys_subsys_pll_initial_mt6899(void)
{
	return connsys_subsys_pll_initial_xtal_26000k_mt6899_gen();
}

int connsys_low_power_setting_mt6899(unsigned int curr_status, unsigned int next_status)
{
	connsys_adie_clock_buffer_setting(curr_status, next_status);

	if (curr_status == 0) {
		connsys_low_power_setting_mt6899_gen();
	} else {
		/* Not first on */
	}
	/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
...
int consys_sema_acquire_timeout_mt6899(unsigned int index, unsigned int usec)
{
	unsigned int i;
	...
	pr_err("Get semaphore 0x%x timeout, g_sema_irq_flags=%lu, dump status:\n", index, g_sema_irq_flags);
...
void consys_sema_release_mt6899(unsigned int index)
{
	...
	if (duration > SEMA_HOLD_TIME_THRESHOLD)
		pr_notice("%s hold semaphore (%d), g_sema_irq_flags=%lu for %lu ms\n",
...
int consys_spi_read_nolock_int_mt6899(enum sys_spi_subsystem subsystem, unsigned int addr,
...
int consys_spi_read_nolock_mt6899(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int *data)
{
	int ret = 0;
	...
	ret = consys_spi_read_nolock_int_mt6899(subsystem, addr, data, spi_base_addr);
	return ret;
}

int consys_spi_read_mt6899(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	int ret = 0;

	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS)
		return consys_spi_read_nolock_mt6899(subsystem, addr, data);
	...
	ret = consys_spi_read_nolock_mt6899(subsystem, addr, data);

	consys_sema_release_mt6899(CONN_SEMA_RFSPI_INDEX);
	return ret;
}

int consys_spi_write_nolock_int_mt6899(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, mapped_addr spi_base_addr)
{
#ifndef CONFIG_FPGA_EARLY_PORTING
	int check = 0;
...
int consys_spi_write_nolock_mt6899(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;
	/* If (Dual A-die) {RFSPI0(with base addr 0x18042XXX)
	 * & RFSPI1(with base addr 0x18046XXX)}(GPS with 6686)
	...
	ret = consys_spi_write_nolock_int_mt6899(subsystem, addr, data, spi_base_addr);
	return ret;
}


int consys_spi_write_mt6899(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;

	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS)
		return consys_spi_write_nolock_mt6899(subsystem, addr, data);
	...
	ret = consys_spi_write_nolock_mt6899(subsystem, addr, data);
...
int consys_spi_update_bits_nolock_mt6899(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, unsigned int mask)
{
	...
	ret = consys_spi_read_nolock_mt6899(subsystem, addr, &curr_val);
	...
	if (change) {
		ret = consys_spi_write_nolock_mt6899(subsystem, addr, new_val);
	}

	return ret;
}

int consys_spi_update_bits_mt6899(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, unsigned int mask)
{
	int ret = 0;

	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS)
		return consys_spi_update_bits_nolock_mt6899(subsystem, addr, data, mask);
	...
	consys_spi_update_bits_nolock_mt6899(subsystem, addr, data, mask);
	consys_sema_release_mt6899(CONN_SEMA_RFSPI_INDEX);

	return ret;
}

int consys_spi_clock_switch_mt6899(enum connsys_spi_speed_type type)
{
	int check;
	int ret = 0;
...
int consys_subsys_status_update_mt6899(bool on, int radio)
{
	...
	if (consys_sema_acquire_timeout_mt6899(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX,
	...
	consys_sema_release_mt6899(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX);
	...
	if (on && (radio == CONNDRV_TYPE_BT) &&
	    ...
	    == 0x0))
		consys_pre_cal_restore_mt6899();

	return 0;
}

bool consys_is_rc_mode_enable_mt6899(void)
{
#ifdef CONFIG_FPGA_EARLY_PORTING
	return false;
#else /* CONFIG_FPGA_EARLY_PORTING */
...
#ifndef CONFIG_FPGA_EARLY_PORTING
void consys_spi_write_offset_range_nolock_mt6899(
	...
	unsigned int reg_offset, unsigned int value_offset, unsigned int size)
{
	...
	data2 = data & (~reg_mask);
	data2 = (data2 | value);
	consys_spi_write_nolock_mt6899(subsystem, addr, data2);
}
...
int connsys_adie_top_ck_en_ctl_mt6899(bool onoff)
{
	return connsys_adie_top_ck_en_ctl_mt6899_gen(onoff);
}
...
static int consys_get_chip_info(char *buf, unsigned int size, int print_log)
{
	...
	splitValues(mt6899_chip_info[1], mt6899_chip_info[2], split_info_list);
...
int consys_get_chip_info_mt6899(char *buf, unsigned int size)
{
