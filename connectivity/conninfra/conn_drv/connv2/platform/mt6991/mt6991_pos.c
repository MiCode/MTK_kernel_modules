...
int consys_conninfra_on_power_ctrl_mt6991(unsigned int enable)
{
	...
	if (enable == 0) {
		...
		/* (sideband mode)  SPM set bus protect CG */
		CONSYS_SET_BIT(vir_addr_0x1c00d000 +
			0xF4, (0x1U << 1));

		/* (sideband mode)  SPM set bus protect */
		CONSYS_SET_BIT(vir_addr_0x1c00d000 +
			0xDC, (0x1U << 1));

		/* (sideband mode)  SPM check bus protect ready */
		check = 0;
		CONSYS_REG_BIT_POLLING(vir_addr_0x1c00d000 +
			0x208,
			1, 1, 0, 1000, check);
		if (check != 0) {
			consys_is_bus_hang_mt6991();
		}

		/* (sideband mode)  SPM set bus protect CG clear */
		CONSYS_SET_BIT(vir_addr_0x1c00d000 +
			0xF8, (0x1U << 1));
