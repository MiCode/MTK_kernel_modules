...
int g_faultb_gpio_mt6989 = 0, g_pmic_en_gpio_mt6989 = 0, g_pmic_exception_no_reset = 0;
...
#define PMIC_EXCEPTION_STRING_LEN	40
	...
	if (g_pmic_exception_no_reset) {
		pr_info("[%s] PMIC exception without reset\n", __func__);
		return IRQ_HANDLED;
	}
...
int connv3_plt_pmic_initial_setting_mt6989(struct platform_device *pdev, struct connv3_dev_cb* dev_cb)
{
	...
	ret = of_property_read_u32(node, "pmic-exception-no-reset", &g_pmic_exception_no_reset);
	if (ret < 0)
		g_pmic_exception_no_reset = 0;
	pr_info("g_pmic_exception_no_reset = %d\n", g_pmic_exception_no_reset);
