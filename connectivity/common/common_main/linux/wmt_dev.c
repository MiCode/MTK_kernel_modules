...
static bool wmt_dev_detect_time_change(void)
{
	static struct timeval prev_utc;
	static UINT64 prev_soc_time;
	static int counter;
	struct timeval curr_utc;
	UINT64 curr_soc_time;
	...
	curr_soc_time = mtk_wcn_consys_timestamp_get(); /* current time = 0, if not support */

	if ((counter != 0) && (curr_soc_time != 0)) {
		...
		if ((soc_utc_diff_ms >= 5000) && (soc_time_diff_ms != 0)) {
