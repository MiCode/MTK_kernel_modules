...
INT32 stp_auto_rst_pause_temporary;
...
VOID mtk_wcn_stp_ctx_restore(void)
{
	...
	if (STP_IS_ENABLE_RST(stp_core_ctx) && stp_auto_rst_pause_temporary == 0)
		stp_btm_notify_wmt_rst_wq(STP_BTM_CORE(stp_core_ctx));
	else
		STP_INFO_FUNC("No to launch whole chip reset! for debugging purpose\n");

	if (stp_auto_rst_pause_temporary == 1) {
		mtk_wcn_stp_emi_dump_flag_ctrl(0);
		stp_auto_rst_pause_temporary = 0;
	}
}
...
VOID mtk_wcn_stp_set_auto_rst_puase_temporary(INT32 pause)
{
	stp_auto_rst_pause_temporary = pause;
}
