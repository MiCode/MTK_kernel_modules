...
static int drv_spi_hopping(void)
{
	...
	if (hw_id == 0x2001) {
		...
		/* Enable WPLL */
		drv_host_read(si, 0x18003004, &val);
		drv_host_write(si, 0x18003004, val | (0x1 << 23));

		/* polling PLL_RDY */
		for (i = 0; i < 100; i++) {
			drv_host_read(si, 0x18001320, &val);
			if (val & 0x00000001) {
				WCN_DBG(FM_NTC | CHIP,
					"%s: POLLING PLL_RDY success !\n", __func__);

				/* enable fm_hs_ck_en */
				drv_host_read(si, 0x18004108, &val);
				drv_host_write(si, 0x18004108, val | (0x3 << 4));
...
static int fm_conninfra_set_reg_top_ck_en(unsigned int val)
{
	...
	while (count < FM_POLLING_LIMIT) {
		fm_delayus(5000);
		drv_host_read(si, 0x18005130, &tem);
		if ((tem & 0xFFFFFFFD) == 0x00000000)
			break;
	}
