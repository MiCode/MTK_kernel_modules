					...
					struct btmtk_sdio_card *data)
{
	...
	if (ptr_name) {
		...
		if (err != 0) {
			...
			(void)snprintf(g_card->bt_cfg_file_name, MAX_BIN_FILE_NAME_LEN, "%s", BT_CFG_NAME);
		}
...
static int btmtk_sdio_download_rom_patch(struct btmtk_sdio_card *card)
{
	...
	if ((u4FwVersion & 0xff) == 0xff) {
		...
		/*     $$$$ : chip id */
		/*     % : fw version + 1 (in HEX) */
		(void)snprintf(uFirmwareName, MAX_BIN_FILE_NAME_LEN, "mt%04x_patch_e%x_hdr.bin",
	...
	if ((fw_firmware->data[8] >= '0') && (fw_firmware->data[8] <= '9'))
		...
		(void)sprintf(fw_version_str, "%.4s-%.2s-%.2s.%.1s.%.2s.%.1s.%.1s.%.2s",
...
patch_end:
	...
	if (ret) {
		BTMTK_ERR("read chipid fail(%d)", ret);
		ret = -EIO;
		goto done;
	}
...
#else
			data = irq_get_irq_data(g_card->wobt_irq);
			if (data)
				desc = irq_data_to_desc(data);
			else
				BTMTK_ERR("%s:can't get data\n", __func__);
#endif
			if (desc)
