...
static int BT_init(void)
{
	...
	if (alloc_ret) {
		/*K19A code for HQ-134298 by zhangpeng at 2021/5/17 start*/
		BT_LOG_PRT_ERR("Failed to register device numbers, move to dynamic alloc\n");
		alloc_ret = alloc_chrdev_region(&dev, 0, BT_devs, BT_DRIVER_NAME);
		BT_major = MAJOR(dev);
		if (alloc_ret) {
			BT_LOG_PRT_ERR("Failed to register device numbers\n");
                  	return alloc_ret;
                }
        	/*K19A code for HQ-134298 by zhangpeng at 2021/5/17 end*/
	}
