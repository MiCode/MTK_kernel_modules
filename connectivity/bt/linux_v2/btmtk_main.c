...
int btmtk_send_assert_cmd(struct btmtk_dev *bdev)
{
...
#else
	/* use send_and_recv make sure drv own for BTMTK_TX_PKT_SEND_DIRECT
	 * evt would be same with cmd
	 * can not use btmtk_main_send_cmd, it would alloc_skb twice
	 */
	{
		u8 drv_own_retry = 10;
		do {

			ret = main_info.hif_hook.send_and_recv(bdev, skb, cmd.content, cmd.len,
	...
	if (ret < 0) {
		...
		if (btmtk_get_chip_state(bdev) != BTMTK_STATE_FW_DUMP)
			...
			BTMTK_INFO("%s: chip state already fw dump state, skip set assert state", __func__);
	}
