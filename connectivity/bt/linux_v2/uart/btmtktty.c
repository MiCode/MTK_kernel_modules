...
static struct wakeup_source *bt_trx_wakelock = NULL;
...
static int btmtk_uart_close(struct hci_dev *hdev)
{
	...
	if (bt_trx_wakelock)
		...
		BTMTK_ERR("%s: bt_trx_wakelock is NULL", __func__);
	...
	while (1) {
		...
		if (sent_len > 0) {
			...
			if (bdev && bdev->interface_state != BTMTK_STATE_DISCONNECT) {
				...
				if (ret <= 0) {
					BTMTK_ERR("%s: tty write failed, ret = %d, zero_pkt_cnt = %d", __func__, ret, zero_pkt_cnt);
					usleep_range(1000, 1100);
				}
				log_time = jiffies_to_msecs(jiffies) - jiffies_to_msecs(log_time);
				if (log_time > TIME_DUMP_OF_FW_PKG_DL)
