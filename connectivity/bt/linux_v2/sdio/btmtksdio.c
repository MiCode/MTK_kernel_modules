...
static atomic_t b_suspending = ATOMIC_INIT(0);
...
static inline void btmtk_sdio_wake_lock(struct wakeup_source *ws, unsigned int msec)
{
	if (ws) {
		if ((ws == g_sdio_dev.main_ws) || (ws == g_sdio_dev.irq_ws)) {
			/*If BT cmd is sent during suspend, main_ws or irq_ws will abort suspend.
			It will results to suspend <--> abort dead loop.
			Since cmd&event is syncly completed if cmd is sent in suspend() function.
			There is no need to hold irq/trx wakelock in suspend procedure*/
			if (atomic_read(&b_suspending) == 1) {
				BTMTK_ERR("%s no lock in suspending", __func__);
				return;
			}
		}
		if (msec > 0)
...
static inline void btmtk_sdio_wake_unlock(struct wakeup_source *ws)
{
	if (ws)
		__pm_relax(ws);
}
		...
		struct debug_reg_struct debug_reg)
{
	...
	for (i = 0; i < count; i++) {
		...
		case 1:
			/* reg read address */
			ret = btmtk_sdio_readl(debug_reg.reg[i].content[0],
				&value,
				cif_dev->func);
			if (ret == -ETIMEDOUT) {
				BTMTK_ERR("%s L %d error abort", __func__, __LINE__);
				break;
			}
		...
		case 2:
			...
			ret = btmtk_sdio_writel(debug_reg.reg[i].content[0],
		...
		case 3:
			...
			ret = btmtk_sdio_readl(debug_reg.reg[i].content[2],
				&value,
				cif_dev->func);
			if (ret == -ETIMEDOUT) {
				BTMTK_ERR("%s L %d error abort", __func__, __LINE__);
...
static int btmtk_sdio_read_register(struct btmtk_dev *bdev, u32 reg, u32 *val)
{
	...
	ret = btmtk_main_send_cmd(bdev, cmd.content, cmd.len, event.content, event.len, DELAY_TIMES,
		...
		int delay, int retry, int pkt_type, bool flag)
{
	...
	ulong flags;
	struct btmtk_sdio_dev *cif_dev = NULL;
	struct data_struct fw_assert_cmd, fw_assert_cmd1, notify_alt_evt;
	...
	btmtk_get_cmd_or_event(bdev, FD5B_ASSERT_CMD, &fw_assert_cmd);
	btmtk_get_cmd_or_event(bdev, WMT_ASSERT_CMD, &fw_assert_cmd1);
	btmtk_get_cmd_or_event(bdev, NOTIFY_ALT_EVT, &notify_alt_evt);
	...
	if (skb->len > 9) {
		if (skb->data[0] == 0x01 && skb->data[1] == 0x6f && skb->data[2] == 0xfc &&
			...
			if (evt_skb) {
				...
				notify_alt_evt.content[3] = (crValue & 0xFF000000) >> 24;
				notify_alt_evt.content[4] = (crValue & 0x00FF0000) >> 16;
				notify_alt_evt.content[5] = (crValue & 0x0000FF00) >> 8;
				notify_alt_evt.content[6] = (crValue & 0x000000FF);
	...
	if ((skb->len == fw_assert_cmd.len &&
			!memcmp(skb->data, fw_assert_cmd.content, fw_assert_cmd.len))
			|| (skb->len == fw_assert_cmd1.len &&
			!memcmp(skb->data, fw_assert_cmd1.content, fw_assert_cmd1.len))) {
...
{
	struct data_struct read_address_event;
	u8 *p = NULL, *pend = NULL;
	u8 attr_len, attr_type;

	btmtk_get_cmd_or_event(bdev, READ_ADDRESS_EVT, &read_address_event);
		...
		if (memcmp(skb->data, &read_address_event.content[1], read_address_event.len - 1) == 0
			...
			if (skb->data[0] == WOBLE_DEBUG_EVT_TYPE) {
				...
				while (p < pend) {
					...
					if (WOBX_TRIGGER_INFO_TRIGGER_CNT_TYPE == attr_type) {
...
static int btmtk_sdio_register_dev(struct btmtk_sdio_dev *cif_dev)
{
...
static int btmtk_sdio_main_thread(void *data)
{
	...
	for (;;) {
		...
		btmtk_sdio_wake_lock(g_sdio_dev.main_ws, 0);
		...
		btmtk_sdio_wake_unlock(g_sdio_dev.main_ws);	//normal unlock
	}

	atomic_set(&cif_dev->sdio_thread.thread_status, 0);
	btmtk_sdio_wake_unlock(g_sdio_dev.main_ws);	//thread stop from break;
	BTMTK_WARN("%s end", __func__);
...
static int btmtk_probe_retry(struct btmtk_dev *bdev)
{
	...
	ret = wait_event_timeout(bdev->probe_fail_wq, test_bit(BT_PROBE_RESET_DONE, &bdev->flags),
					...
					const struct sdio_device_id *id)
{
	...
	while (!atomic_read(&cif_dev->sdio_thread.thread_status) && i < CHECK_THREAD_RETRY_TIMES) {
		...
		if (i == RETRY_TIMES - 1) {
			...
			bdev->bt_cfg.support_dongle_whole_reset = 1;
...
LOAD_PATCH_RETRY:
	...
	cif_dev->patched = bmain_info->hif_hook_chip.patched;
...
static int btmtk_cif_suspend(struct device *dev)
{
	...
	struct btmtk_main_info *main_info = btmtk_get_main_info();

	BTMTK_INFO("%s, enter", __func__);

	if (!dev) {
		BTMTK_ERR("%s: dev is null", __func__);
		return 0;
	}
	...
	atomic_set(&b_suspending, 1);
	ret = btmtk_sdio_keep_driver_own(cif_dev, 1);
	if (ret == -ETIMEDOUT) {
		BTMTK_ERR("%s set driver own timeout", __func__);
		goto suspend_end;
	}
	...
	ret = btmtk_sdio_keep_driver_own(cif_dev, 0);
	if (ret == -ETIMEDOUT) {
		BTMTK_ERR("%s set fw own timeout", __func__);
		goto suspend_end;
	}
suspend_end:
	btmtk_sdio_delete_fw_own_timer(cif_dev);
	/* Set End/Error state */
	if (ret == 0)
		btmtk_set_chip_state((void *)bdev, cif_state->ops_end);
	else {
		bdev->suspend_count--; //When return failure, suspend will abort without calling the resume callback. should decrease the count.
		btmtk_set_chip_state((void *)bdev, cif_state->ops_error);
		BTMTK_ERR("%s suspend fail ret=%d, trigger reset", __func__, ret);
		if (ret == -ETIMEDOUT)
			main_info->chip_reset_flag = 1;
		btmtk_handle_mutex_unlock(bdev);
		btmtk_reset_trigger(bdev);
		btmtk_handle_mutex_lock(bdev);
	}
	atomic_set(&b_suspending, 0);
...
reset_retry:
	...
	do {
		 ...
		 */
		btmtk_sdio_keep_driver_own(cif_dev, 1);
		if (!bmain_info->hif_hook_chip.patched)
			break;
		ret = btmtk_sdio_readl(PD2HRM0R, &u32ReadCRValue, cif_dev->func);
		msleep(DELAY_TIMES);
		retry--;
...
int btmtk_cif_register(void)
{
	...
	g_sdio_dev.main_ws = wakeup_source_register(NULL, "btmtksdio_main_ws");
	g_sdio_dev.irq_ws = wakeup_source_register(NULL, "btmtksdio_irq_ws");
	g_sdio_dev.device_ws = wakeup_source_register(NULL, "btmtksdio_device_ws");
#elif (defined(LINUX_OS) && (KERNEL_VERSION(5, 4, 0) > LINUX_VERSION_CODE)) ||	\
		(defined(ANDROID_OS) && (KERNEL_VERSION(4, 19, 0) > LINUX_VERSION_CODE))
	g_sdio_dev.main_ws = wakeup_source_register("btmtksdio_main_ws");
	g_sdio_dev.irq_ws = wakeup_source_register("btmtksdio_irq_ws");
	g_sdio_dev.device_ws = wakeup_source_register("btmtksdio_device_ws");
#else
	g_sdio_dev.main_ws = wakeup_source_register(NULL, "btmtksdio_main_ws");
	g_sdio_dev.irq_ws = wakeup_source_register(NULL, "btmtksdio_irq_ws");
	g_sdio_dev.device_ws = wakeup_source_register(NULL, "btmtksdio_device_ws");
...
int btmtk_cif_deregister(void)
{
	...
	wakeup_source_unregister(g_sdio_dev.main_ws);
	wakeup_source_unregister(g_sdio_dev.irq_ws);
	wakeup_source_unregister(g_sdio_dev.device_ws);
	BTMTK_INFO("%s: Done", __func__);
	return 0;
}
