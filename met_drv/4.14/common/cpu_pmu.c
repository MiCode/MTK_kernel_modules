...
#define	PMU_OVERFLOWED_MASK	PMU_OVSR_MASK

static u32 pmu_read_clear_overflow_flag(void)
{
	u32 value;
...
static irqreturn_t perf_event_handle_irq_ignore_overflow(int irq_num, void *dev)
{
	...
	pmovsr = pmu_read_clear_overflow_flag();
...
static unsigned long perf_event_get_evttype(struct perf_event *ev) {
	...
	for_each_possible_cpu(cpu) {
		...
		for (i = 0; i < nr_events; i++) {
			...
			if (met_cpu_pmu_method) {
				...
				if (ev_attr_p && perf_event_get_evttype(ev) == ev_attr_p->id)
					is_cpu_cycle_evt = 1;
			}
