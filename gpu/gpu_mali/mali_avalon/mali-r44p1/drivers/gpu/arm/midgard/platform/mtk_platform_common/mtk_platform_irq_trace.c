...
unsigned long long mtk_irq_trace_data[IRQ_DATA_IDX_NUM];
unsigned int irq_monitor_default_enabled = false;
unsigned int irq_monitor_override_threshold_ms = DEFAULT_GPU_TH1_MS;

void mtk_debug_irq_trace_record_data(enum IRQ_DATA_IDX data_id, unsigned long long data)
{
	if (data_id < IRQ_DATA_IDX_NUM)
...
static void mtk_debug_irq_trace_show(void)
{
	...
	pr_info("GPU_IRQ_status: 0x%llx\n", mtk_irq_trace_data[IRQ_DATA_IDX_GPU_IRQ_STATUS]);
	pr_info("JOB_IRQ_status: 0x%llx\n", mtk_irq_trace_data[IRQ_DATA_IDX_JOB_IRQ_STATUS]);
