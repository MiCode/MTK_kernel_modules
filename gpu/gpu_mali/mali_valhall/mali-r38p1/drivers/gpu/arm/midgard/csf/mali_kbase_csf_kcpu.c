			...
			struct kbase_ioctl_kcpu_queue_new *newq)
{
	...
	if (idx >= (int)KBASEP_MAX_KCPU_QUEUES) {
		dev_err(kctx->kbdev->dev, "mali: %s, %d: find_first_zero_bit() return %d ", __func__, __LINE__, idx);
		ret = -ENOMEM;
		goto out;
	}

	if (WARN_ON(kctx->csf.kcpu_queues.array[idx])) {
		dev_err(kctx->kbdev->dev, "mali: %s, %d: kcpu_queues.array warn_on triggered", __func__, __LINE__);
		ret = -EINVAL;
		goto out;
	}
	...
	if (!queue) {
		dev_err(kctx->kbdev->dev, "mali: %s, %d: kzalloc() return null", __func__, __LINE__);
		ret = -ENOMEM;
		goto out;
	}
	...
	if (queue->wq == NULL) {
		dev_err(kctx->kbdev->dev, "mali: %s, %d: alloc_workqueue() return null", __func__, __LINE__);
		kfree(queue);
		ret = -ENOMEM;
